...
## Unreleased: Simplified Chinese UI (zh_CN)

- **The GUI is translatable and ships a Simplified Chinese translation.** Every user-facing
  string in `gui/` now goes through `tr()` / `QObject::tr()`, and
  `translations/cheatengine_zh_CN.ts` (1144 strings) is compiled to
  `translations/cheatengine_zh_CN.qm`. With a `zh_CN` locale the whole window, its menus,
  dialogs, panels and tooltips come up in Chinese; any other locale keeps the built-in
  English strings (an untranslated string simply falls through).
- **Settings → Languages** now works: *System default* / *English* / *简体中文* is stored in
  `ui/language` and applied on the next start (previously the combo persisted a value that
  nothing read).
- **Build:** the `.qm` files are committed, so a build without Qt's Linguist tools still
  installs working translations. When `lrelease` is present (Qt6 `LinguistTools`) the `.qm`
  files are recompiled from the `.ts` at build time and a fresh copy is dropped next to the
  binary, so editing a `.ts` and rebuilding is all it takes. Installed to
  `<prefix>/share/cheatengine/translations` (`/usr/bin/cheatengine` finds it through the new
  `<exe>/../share/cheatengine/translations` search path).
- **Deliberately not translated:** the `cescan` CLI (Qt-free, prints via `std::cout`, would
  need gettext), `cecore`'s internal error/diagnostic strings, and the value-type names in the
  cheat table's Type column (`Byte`, `4 Bytes`, …): those round-trip through saved tables, so
  they stay canonical English.
- **Fixed: installing the .deb broke the Speedhack dialog and the Mono dissector.**
  Both load a helper library at run time instead of linking it, so neither was ever
  packaged: `libspeedhack.so` (the GUI dlopen's `<exe-dir>/libspeedhack.so` into the target, so
  "Apply" failed with *"dlopen returned … but libspeedhack.so is not mapped in the target"*)
  and `libcecore_mono_agent.so` (injected into Mono/IL2CPP targets;
  `ce::findMonoAgentPath()` also searches `<prefix>/lib`). The install rules now ship
  `libspeedhack.so` next to the binaries and the Mono agent in the lib dir; the AppImage script
  copies the speedhack library into `AppDir/usr/bin/` for the same reason (linuxdeploy cannot
  see a library that is only dlopen'd).
- **Fixed: "Apply" in the Speedhack dialog failed on a sandboxed target (Steam Linux Runtime /
  pressure-vessel, Flatpak) with *"dlopen returned 18446744073709551104 but libspeedhack.so is
  not mapped in the target"*.** Two independent bugs, both invisible outside a mount-namespace
  sandbox:
  - `ce::resolveProcPath` treated "this path exists on the host too" as "the same file". A
    sandbox *overlays* standard paths with its own tree, so the target's
    `/usr/lib/x86_64-linux-gnu/libc.so.6` was parsed from the **host's** copy: every symbol
    offset was wrong for the target (`dlopen` resolved to `pthread_rwlock_rdlock+0x10` on a
    Proton game), the injected call returned garbage (hence the `18446744073709551104` =
    `-512` handle) and no library was ever mapped. Paths are now compared by `st_dev`/`st_ino`
    and the file the target really sees (`/proc/<pid>/root<path>`) wins whenever the two differ.
  - The library path is opened by the **target's** dynamic linker, so `/usr/bin/libspeedhack.so`
    — shadowed inside a pressure-vessel container by the runtime's own `/usr` — was ENOENT
    there. `injectLibrary` now tries the path as-is, through the host mount the sandbox exposes
    (`/run/host`, used by Steam's runtime and Flatpak) and through the tracer's own root, and
    keeps the first candidate that actually gets the library mapped.
...
- **`.CT` hotkeys are now CE-compatible both ways (CE parity).** CE stores a record's hotkeys as
  a nested `<Hotkeys><Hotkey><Action>N</Action><Keys><Key>vk</Key>...</Keys></Hotkey>...` structure
  with Windows virtual-key codes; we only understood our own flat format, so a CE table's hotkeys
  imported as garbage and CE couldn't read ours. The loader now maps each `Action` (toggle / set
  value / increase / decrease) to the matching hotkey slot and translates the VK codes to a Qt key
  sequence (`Ctrl+H`, `F5`, `Ctrl+Up`); the saver writes the same nested structure, so tables
  round-trip through Cheat Engine. Our old flat format still loads (auto-detected).
...
- **Step from the Memory Viewer (CE parity).** The Memory Viewer's Debug menu and toolbar
  ...
  is the debug view. They delegate to the debug session the Debugger window owns, so the
  current-instruction line in the viewer advances on each stop. Run-to-cursor targets the
  viewer's own selected line (a one-shot breakpoint). All are no-ops while nothing is paused.
...
- **Fixed: inline-editing a pointer record's value could write to a stale address**: editing the
  ...
  old target. `setValue` now re-resolves the address expression before writing, so every value
  path (inline edit, "Set value", step hotkeys, freeze) is consistent.
