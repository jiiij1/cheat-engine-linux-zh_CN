...
AdvancedOptionsWindow::AdvancedOptionsWindow(ce::ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Code list/Pause"));   // CE AdvancedOptionsUnit caption

    auto* central = new QWidget;
    setCentralWidget(central);
    auto* v = new QVBoxLayout(central);

    v->addWidget(new QLabel(QObject::tr("Code list:")));       // CE Label

    table_ = new QTableWidget(0, 2);               // CE lvCodelist (Address, Name)
    table_->setHorizontalHeaderLabels({QObject::tr("Address"), QObject::tr("Name")});
...
void AdvancedOptionsWindow::showContextMenu(const QPoint& pos) {
    ...
    QMenu m(this);
    auto* openAct    = m.addAction(QObject::tr("Open the disassembler at this location"));
    m.addSeparator();
    auto* nopAct     = m.addAction(QObject::tr("Replace with code that does nothing"));
    auto* restoreAct = m.addAction(QObject::tr("Restore with original code"));
    m.addSeparator();
    auto* renameAct  = m.addAction(QObject::tr("Rename"));
    auto* removeAct  = m.addAction(QObject::tr("Remove from list"));
    ...
    if (chosen == openAct) {
        ...
        else QMessageBox::warning(this, "NOP", QObject::tr("Could not read/patch the instruction."));
        ...
        auto name = QInputDialog::getText(this, QObject::tr("Rename"), QObject::tr("Name:"), QLineEdit::Normal, cur, &ok);
