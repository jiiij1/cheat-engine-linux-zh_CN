...
BranchMapper::BranchMapper(ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Branch Mapper (LBR)"));
    resize(900, 600);
    ...
    form->addRow(QObject::tr("Thread ID:"), tidSpin_);

    auto* btnRow = new QHBoxLayout;
    startBtn_ = new QPushButton(QObject::tr("Start sampling"));
    stopBtn_  = new QPushButton(QObject::tr("Stop"));
    stopBtn_->setEnabled(false);
    clearBtn_ = new QPushButton(QObject::tr("Clear"));
    statusLabel_ = new QLabel(
        LbrTracer::available() ? QObject::tr("Idle") :
        QObject::tr("perf_event_open(BRANCH_STACK) unavailable; needs CAP_SYS_ADMIN or "
    ...
    table_->setHorizontalHeaderLabels({QObject::tr("From"), QObject::tr("From symbol"), QObject::tr("To"), QObject::tr("To symbol"), QObject::tr("Hits"), QObject::tr("% of total")});
...
void BranchMapper::onStart() {
    if (!LbrTracer::available()) {
        QMessageBox::warning(this, QObject::tr("LBR unavailable"),
            QObject::tr("perf_event_open(BRANCH_STACK) failed. Need CAP_SYS_ADMIN or "
            "sysctl kernel.perf_event_paranoid <= 1 (run as root, or set it)."));
        return;
    }
    pid_t tid = (pid_t)tidSpin_->value();
    if (!tracer_.start(tid)) {
        QMessageBox::warning(this, QObject::tr("Start failed"),
            QObject::tr("Could not attach LBR sampling to that thread."));
        return;
    }
    ...
    statusLabel_->setText(QObject::tr("Sampling…"));
...
void BranchMapper::onStop() {
    ...
    statusLabel_->setText(QObject::tr("Stopped. %1 samples total.").arg(totalSamples_));
}
...
void BranchMapper::onClear() {
    ...
    statusLabel_->setText(QObject::tr("Cleared."));
}
...
void BranchMapper::onPoll() {
    ...
    for (size_t i = 0; i < flat.size(); ++i) {
        ...
        table_->setItem((int)i, 5, new QTableWidgetItem(QString::number(pct, 'f', 2) + QObject::tr("%")));
    }
    statusLabel_->setText(QObject::tr("Sampling… %1 total / %2 unique pairs")
