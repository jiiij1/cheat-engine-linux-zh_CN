...
ChangeAddressDialog::ChangeAddressDialog(const QString& address, ce::ValueType type,
    ...
    setWindowTitle(QObject::tr("Change address"));
    ...
    form->addRow(QObject::tr("Address:"), addrEdit_);
    ...
    form->addRow(QObject::tr("Type:"), typeCombo_);

    lengthEdit_ = new QLineEdit(QString::number(length > 0 ? length : 1));  // CE edtSize
    form->addRow(QObject::tr("Length:"), lengthEdit_);          // CE lengthlabel — for String/Array
    v->addLayout(form);

    auto* flags = new QHBoxLayout;
    pointerCheck_ = new QCheckBox("Pointer");      // CE cbPointer
    hexCheck_     = new QCheckBox(QObject::tr("Hexadecimal"));  // CE cbHex
    hexCheck_->setChecked(showHex);
    signedCheck_  = new QCheckBox(QObject::tr("Signed"));       // CE cbSigned
    signedCheck_->setChecked(showSigned);
    unicodeCheck_ = new QCheckBox(QObject::tr("Unicode"));      // CE cbunicode
    ...
    baseRow->addWidget(new QLabel(QObject::tr("Base:")));
    ...
    auto* addOffBtn = new QPushButton(QObject::tr("Add offset"));
...
void ChangeAddressDialog::updatePreview() {
    ...
    if (!addr || !*addr) {
        previewLabel_->setText(QObject::tr("→ ?? (does not resolve)"));
        return;
    }
...
void ChangeAddressDialog::addOffsetRow(long long value) {
    ...
    h->addWidget(new QLabel(QObject::tr("Offset:")));
    auto* edit = new QLineEdit;
    edit->setText(value < 0 ? QObject::tr("-") + QString::number(-value, 16) : QString::number(value, 16));
    h->addWidget(edit);
    auto* rm = new QPushButton(QObject::tr("−"));
    rm->setFixedWidth(28);
    h->addWidget(rm);
