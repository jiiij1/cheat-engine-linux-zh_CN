...
DebuggerWindow::DebuggerWindow(ce::ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Debugger"));
    resize(920, 640);
    ...
    contBtn_   = new QPushButton(QObject::tr("Continue"));     contBtn_->setShortcut(Qt::Key_F9);
    intoBtn_   = new QPushButton(QObject::tr("Step Into"));    intoBtn_->setShortcut(Qt::Key_F7);
    overBtn_   = new QPushButton(QObject::tr("Step Over"));    overBtn_->setShortcut(Qt::Key_F8);
    outBtn_    = new QPushButton(QObject::tr("Step Out"));
    rtcBtn_    = new QPushButton(QObject::tr("Run to Cursor")); rtcBtn_->setShortcut(Qt::Key_F4);
    detachBtn_ = new QPushButton(QObject::tr("Detach"));
    ...
    excBtn->setText(QObject::tr("Break on exceptions ▾"));
    excBtn->setPopupMode(QToolButton::InstantPopup);
    auto* excMenu = new QMenu(excBtn);
    struct SigEntry { QString name; int sig; };
    static const SigEntry kSigs[] = {
        {QObject::tr("SIGSEGV (segfault)"), SIGSEGV}, {QObject::tr("SIGILL (illegal instruction)"), SIGILL},
        {QObject::tr("SIGFPE (FP / divide-by-zero)"), SIGFPE}, {QObject::tr("SIGBUS (bus error)"), SIGBUS},
        {QObject::tr("SIGABRT (abort)"), SIGABRT}, {"SIGTRAP", SIGTRAP},
    };
    ...
    statusLabel_ = new QLabel(QObject::tr("Not attached"));
    ...
    memAddrInput_->setPlaceholderText(QObject::tr("memory address (hex), then Enter"));
    ...
    threadRow->addWidget(new QLabel(QObject::tr("Thread:")));
    ...
    flagsLabel_ = new QLabel(QObject::tr("Flags:"));
    flagsLabel_->setFont(mono);
    flagsLabel_->setToolTip(QObject::tr("Status/control flags set in RFLAGS at the current stop"));
    rl->addWidget(flagsLabel_);

    auto* bpGroup = new QGroupBox(QObject::tr("Breakpoints"));
    auto* bl = new QVBoxLayout(bpGroup);
    auto* addRow = new QHBoxLayout();
    bpInput_ = new QLineEdit();
    bpInput_->setPlaceholderText(QObject::tr("address (hex), then Add"));
    auto* addBtn = new QPushButton(QObject::tr("Add"));
    auto* dataBpBtn = new QPushButton(QObject::tr("Data BP..."));
    dataBpBtn->setToolTip(QObject::tr("Hardware watchpoint: break when an address is written or accessed"));
    addRow->addWidget(bpInput_);
    addRow->addWidget(addBtn);
    addRow->addWidget(dataBpBtn);
    bl->addLayout(addRow);
    bpList_ = new QListWidget();
    bpList_->setToolTip(QObject::tr("Check to enable/disable · double-click to set/edit its condition"));
    ...
    connect(bpList_, &QListWidget::itemChanged, this, [this](QListWidgetItem* it) {
        ...
        if (!session_->isAttached() || !session_->isStopped()) {
            ...
            statusLabel_->setText(QObject::tr("Stop the target to enable/disable breakpoints"));
            return;
        }
        if (wantEnabled) {
            ...
            if (id <= 0) {
                ...
                statusLabel_->setText(QObject::tr("Could not enable breakpoint"));
                return;
            }
            bps_[row].id = id;
    ...
    auto* rmBtn = new QPushButton(QObject::tr("Remove selected"));
    bl->addWidget(rmBtn);
    ...
    if (proc_ && session_->attach(proc_->pid(), proc_)) {
        ...
        statusLabel_->setText(QObject::tr("Attach failed (need ptrace permission?)"));
...
void DebuggerWindow::onContinue() {
    if (!session_->isAttached()) return;
    statusLabel_->setText(QObject::tr("Running..."));
    setRunningUi(true);
...
void DebuggerWindow::onDetach() {
    ...
    statusLabel_->setText(QObject::tr("Detached"));
    setRunningUi(false, true);
}
...
void DebuggerWindow::onAddBreakpoint() {
    ...
    if (!ok || !addr) { statusLabel_->setText(QObject::tr("Invalid breakpoint address")); return; }
...
void DebuggerWindow::addBreakpointAt(uintptr_t addr, const QString& condition) {
    if (!session_->isAttached() || !session_->isStopped()) {
        statusLabel_->setText(QObject::tr("Stop the target before setting a breakpoint"));
        return;
    }
    ...
    if (id <= 0) { statusLabel_->setText(QObject::tr("Failed to set breakpoint at ") + hex(addr)); return; }
...
QString DebuggerWindow::bpRowText(const Bp& b) const {
    ...
    if (b.hitCount > 0) base += QObject::tr("  (hits: %1)").arg(b.hitCount);
    return base;
}
...
void DebuggerWindow::setConditionalBreakpointAtCursor() {
    ...
    if (!addr) return;
    bool ok = false;
    QString cond = QInputDialog::getText(this, QObject::tr("Conditional breakpoint"),
        QObject::tr("Break only when this expression is true\n"
...
void DebuggerWindow::editBreakpointCondition() {
    ...
    QString cond = QInputDialog::getText(this, QObject::tr("Edit breakpoint condition"),
        QObject::tr("Break only when this expression is true (empty = unconditional):"),
...
void DebuggerWindow::onAddDataBreakpoint() {
    ...
    bool ok = false;
    QString addrStr = QInputDialog::getText(this, QObject::tr("Data breakpoint"),
        QObject::tr("Address to watch (hex):"), QLineEdit::Normal,
    ...
    if (!ok || !addr) { statusLabel_->setText(QObject::tr("Invalid address")); return; }

    QStringList types = {QObject::tr("On write"), QObject::tr("On read/write (access)")};
    QString typeSel = QInputDialog::getItem(this, QObject::tr("Data breakpoint"), QObject::tr("Break:"), types, 0, false, &ok);
    if (!ok) return;
    int type = (types.indexOf(typeSel) == 1) ? 3 : 1;   // 1=write, 3=access

    QStringList sizes = {"1", "2", "4", "8"};
    QString sizeSel = QInputDialog::getItem(this, QObject::tr("Data breakpoint"), QObject::tr("Size (bytes):"), sizes, 2, false, &ok);
    if (!ok) return;
    ...
    if (id <= 0) {
        statusLabel_->setText(QObject::tr("No free debug register (max 4 hardware breakpoints)"));
        return;
    }
...
void DebuggerWindow::onDebugEvent(int type) {
    if (type == static_cast<int>(ce::DebugEventType::ProcessExited)) {
        statusLabel_->setText(QObject::tr("Process exited"));
        setRunningUi(false, true);
        return;
    }
...
void DebuggerWindow::refreshStopped() {
    ...
    statusLabel_->setText(QObject::tr("Stopped at %1 (tid %2)")
...
void DebuggerWindow::updateThreadList() {
    ...
    for (size_t i = 0; i < tids.size(); ++i) {
        threadCombo_->addItem(QObject::tr("tid %1").arg(static_cast<int>(tids[i])),
...
void DebuggerWindow::updateMemoryView(uintptr_t addr) {
    ...
    size_t n = (r && *r) ? *r : 0;
    if (n == 0) { memView_->setPlainText(QObject::tr("  <unreadable at ") + hex(addr) + QObject::tr(">")); memChanged_.clear(); return; }
    ...
    QString html = QObject::tr("<pre style=\"margin:0\">");
        ...
        for (size_t i = 0; i < 16; ++i) {
            if (row + i < n) {
                ...
                html += memChanged_[row + i]
                    ? QObject::tr("<span style=\"color:%1\">%2</span>").arg(changedCol, bs)
                    : bs;
...
void DebuggerWindow::updateRegisters(const ce::CpuContext& c) {
    ...
    if (flagsLabel_) {
        ...
        flagsLabel_->setText(QObject::tr("Flags: ") +
...
void DebuggerWindow::updateDisassembly(const ce::CpuContext& c) {
    ...
    if (rr && *rr > 0) {
        ...
        for (auto& in : insns) {
            ...
            if (in.address == c.rip && in.mnemonic != "jmp") {
                if (auto jt = ce::conditionalJumpTaken(in.mnemonic, c.rflags))
                    anno += *jt ? QObject::tr("   (will jump)") : QObject::tr("   (no jump)");
            }
        ...
        out = QObject::tr("  <unable to read code at ") + hex(c.rip) + ">";
    }
...
void DebuggerWindow::onDisasmContextMenu(const QPoint& pos) {
    ...
    QMenu menu(this);
    menu.addAction(QObject::tr("Set breakpoint here"), this, &DebuggerWindow::setBreakpointAtCursor);
    menu.addAction(QObject::tr("Set conditional breakpoint..."), this, &DebuggerWindow::setConditionalBreakpointAtCursor);
    menu.addAction(QObject::tr("Replace with NOPs"),  this, &DebuggerWindow::nopInstructionAtCursor);
    menu.addSeparator();
    // Run to this line (CE's F4): only meaningful while the target is paused.
    QAction* rtc = menu.addAction(QObject::tr("Run to cursor"), this, &DebuggerWindow::onRunToCursor);
