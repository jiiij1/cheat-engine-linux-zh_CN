...
FormDesigner::FormDesigner(QWidget* parent) : QMainWindow(parent) {
    setWindowTitle(QObject::tr("Form Designer"));
    resize(1100, 720);
    ...
    paletteCol->addWidget(new QLabel(QObject::tr("<b>Palette</b>")));
    ...
    canvas_ = new QWidget;
    {
        ...
        canvas_->setStyleSheet(QObject::tr("background-color: %1; border: 1px dashed %2;")
    ...
    canvasLayout->addWidget(new QLabel(QObject::tr("<b>Canvas</b>: click a widget on the canvas or in the Items list to select it")));
    ...
    luaHeader->addWidget(new QLabel(QObject::tr("<b>Generated Lua</b>")));
    auto* regenBtn = new QPushButton(QObject::tr("Refresh"));
    ...
    propsCol->addWidget(new QLabel(QObject::tr("<b>Form</b>")));
    {
        ...
        form->addRow(QObject::tr("Width:"), fw);
        form->addRow(QObject::tr("Height:"), fh);
        propsCol->addLayout(form);
    }

    propsCol->addWidget(new QLabel(QObject::tr("<b>Selected widget</b>")));
    ...
    propForm->addRow(QObject::tr("Name:"),    nameEdit_);
    propForm->addRow(QObject::tr("Caption:"), captionEdit_);
    propForm->addRow("X:",       xSpin_);
    propForm->addRow("Y:",       ySpin_);
    propForm->addRow(QObject::tr("Width:"),   wSpin_);
    propForm->addRow(QObject::tr("Height:"),  hSpin_);
    ...
    propsCol->addStretch();
    auto* saveBtn = new QPushButton(QObject::tr("Save layout (.json)…"));
    auto* loadBtn = new QPushButton(QObject::tr("Load layout (.json)…"));
    auto* exportBtn = new QPushButton(QObject::tr("Copy Lua to clipboard"));
...
void FormDesigner::syncPropertiesFromSelection() {
    if (selected_ < 0 || selected_ >= (int)items_.size()) {
        nameEdit_->setText(QObject::tr("")); captionEdit_->setText(QObject::tr(""));
...
void FormDesigner::onSave() {
    auto path = QFileDialog::getSaveFileName(this, QObject::tr("Save form layout"), "form.json", QObject::tr("JSON (*.json)"));
...
void FormDesigner::onLoad() {
    auto path = QFileDialog::getOpenFileName(this, QObject::tr("Load form layout"), "", QObject::tr("JSON (*.json)"));
    if (path.isEmpty()) return;
    QFile f(path);
    ...
    if (!doc.isObject()) {
        QMessageBox::warning(this, QObject::tr("Load failed"), QObject::tr("Not a valid form layout file."));
        return;
    }
...
void FormDesigner::onGenerateLua() {
    QString out;
    out += QObject::tr("-- Generated by cecore Form Designer\n");
    out += QObject::tr("local form = createForm(true)\n");
    out += QObject::tr("form.Caption = \"Form\"\n");
    out += QObject::tr("form.Width = %1\n").arg(formWidth_);
    out += QObject::tr("form.Height = %1\n").arg(formHeight_);
    for (const auto& it : items_) {
        QString creator;
