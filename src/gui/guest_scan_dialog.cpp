...
GuestScanDialog::GuestScanDialog(ce::ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Emulator guest scan"));
    resize(600, 520);
    ...
    auto* header = new QLabel(profile_.guestCandidates.empty()
        ? QObject::tr("<b>No emulator guest RAM detected.</b> Attach to a recognized "
                  "emulator (Dolphin, PCSX2, RPCS3, DuckStation, …) first.")
        : QObject::tr("<b>%1</b> guest memory. Scan by value, or Unknown Scan then narrow by "
    ...
    for (const auto& g : profile_.guestCandidates)
        regionCombo_->addItem(QObject::tr("0x%1  (%2 MB)").arg(g.base, 0, 16).arg(g.size >> 20));
    form->addRow(QObject::tr("Guest RAM region:"), regionCombo_);

    typeCombo_ = new QComboBox;
    typeCombo_->addItems({"Byte", "2 Bytes", "4 Bytes", "8 Bytes", "Float", "Double"});
    typeCombo_->setCurrentIndex(2);
    form->addRow(QObject::tr("Value type:"), typeCombo_);

    valueEdit_ = new QLineEdit;
    valueEdit_->setPlaceholderText(QObject::tr("value to find (decimal or 0x…)"));
    form->addRow(QObject::tr("Value:"), valueEdit_);

    bigEndianCheck_ = new QCheckBox(QObject::tr("Big-endian (PS3 / Wii / GameCube)"));
    form->addRow("", bigEndianCheck_);
    layout->addLayout(form);

    auto* btnRow = new QHBoxLayout;
    firstBtn_   = new QPushButton(QObject::tr("First Scan"));
    unknownBtn_ = new QPushButton(QObject::tr("Unknown Scan"));
    nextBtn_    = new QPushButton(QObject::tr("Next Scan"));
    newBtn_     = new QPushButton(QObject::tr("New Scan"));
    for (auto* b : {firstBtn_, unknownBtn_, nextBtn_, newBtn_}) btnRow->addWidget(b);
    layout->addLayout(btnRow);

    auto* cmpRow = new QHBoxLayout;
    changedBtn_   = new QPushButton(QObject::tr("Changed"));
    unchangedBtn_ = new QPushButton(QObject::tr("Unchanged"));
    increasedBtn_ = new QPushButton(QObject::tr("Increased"));
    decreasedBtn_ = new QPushButton(QObject::tr("Decreased"));
    ...
    resultsTable_->setHorizontalHeaderLabels({QObject::tr("Guest address"), QObject::tr("Host address"), QObject::tr("Value")});
...
void GuestScanDialog::onNewScan() {
    ...
    statusLabel_->setText(QObject::tr("Ready."));
    refreshResults();
}
...
void GuestScanDialog::refreshResults() {
    if (haveScan_ && !unknownMode_) {
        ...
        statusLabel_->setText(QObject::tr("%1 result(s)%2").arg(candidates_.size())
            .arg(candidates_.size() > shown ? QObject::tr(" (showing first %1)").arg(shown) : QString()));
    } else if (unknownMode_) {
        resultsTable_->setRowCount(0);
        statusLabel_->setText(QObject::tr("Snapshot taken (%1 MB). Change the value in-game, then "
...
void GuestScanDialog::onAddToList() {
    ...
    for (int row : rows) {
        ...
        emit addressSelected(gv.toHost(g), scanType_, scanBigEndian_,
                             QObject::tr("guest 0x%1").arg(regionGuestBase_ + g, 0, 16));
    }
}
