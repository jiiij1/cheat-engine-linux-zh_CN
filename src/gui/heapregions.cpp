...
QString heapKind(const MemoryRegion& region) {
    if (region.path.find("[heap]") != std::string::npos)
        return QObject::tr("brk heap");
    return QObject::tr("anonymous private");
}
...
HeapRegionsWindow::HeapRegionsWindow(ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Heap Regions"));
    resize(760, 480);
    ...
    table_->setHorizontalHeaderLabels({QObject::tr("Start"), QObject::tr("End"), QObject::tr("Size"), QObject::tr("Perm"), QObject::tr("Kind"), QObject::tr("Path")});
...
void HeapRegionsWindow::populate() {
    ...
    setWindowTitle(QObject::tr("Heap Regions - %1 regions, %2 MB")
        .arg(heaps.size())
        .arg(total / 1048576));
}
