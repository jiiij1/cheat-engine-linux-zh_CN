...
LuaConsole::LuaConsole(LuaEngine* engine, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Lua Console"));
    resize(600, 400);
    ...
    output_->appendPlainText(QObject::tr("-- Cheat Engine Lua Console --"));
    output_->appendPlainText(QObject::tr("-- Functions: readInteger(addr), writeInteger(addr, val), getModuleBase(name), ..."));
    ...
    input_->setPlaceholderText(QObject::tr("Enter Lua code, then press Run or Enter…"));
    ...
    auto* runBtn = new QPushButton(QObject::tr("Run"));
    runBtn->setToolTip(QObject::tr("Execute the Lua code (Enter)"));
    connect(runBtn, &QPushButton::clicked, this, &LuaConsole::onExecute);
    inputRow->addWidget(runBtn);

    auto* clearBtn = new QPushButton(QObject::tr("Clear"));
    clearBtn->setToolTip(QObject::tr("Clear the console output"));
...
void LuaConsole::onExecute() {
    ...
    appendColored(output_, QObject::tr("> ") + code, QColor(0x80, 0x86, 0x94));
    input_->clear();

    auto err = engine_->execute(code.toStdString());
    if (!err.empty())
        // Errors in red (legible on both the light and dark themes).
        appendColored(output_, QObject::tr("ERROR: ") + QString::fromStdString(err), QColor(0xd6, 0x2b, 0x2b));
}
