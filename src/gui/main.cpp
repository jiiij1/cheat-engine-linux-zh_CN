...
int main(int argc, char* argv[]) {
    ...
    // and one for the GUI's own translations. The GUI .qm files live under
    // translations/cheatengine_<locale>.qm (next to the binary, or in
    // <prefix>/share/cheatengine/translations). The load is best-effort and
    // silently skips when no file matches, so an untranslated locale (or an
    // uninstalled .qm) simply falls back to the built-in English strings.
    QLocale locale;
    {
        ...
        // The Settings dialog's Languages page stores the choice here
        // ("System default" / "English" / "简体中文"); it overrides the
        // environment locale on the next start.
        const QString langSetting = QSettings().value("ui/language", "System default").toString();
        if (langSetting == QLatin1String("English"))
            locale = QLocale(QLocale::English);
        else if (langSetting == QString::fromUtf8("简体中文"))
    ...
    auto* ceTranslator = new QTranslator(&app);
    {
        ...
        //   <exe-dir>/translations/, <exe-dir>/../translations/, the installed
        //   <exe-dir>/../share/cheatengine/translations/, and the current dir.
        ...
        if (exeLenLocal > 0) {
            ...
            searchDirs << (exeDir + "/translations")
                       << (exeDir + "/../translations")
                       << (exeDir + "/../share/cheatengine/translations");
        }
