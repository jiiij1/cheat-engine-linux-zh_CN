...
static QString foundLabelText(size_t count) {
    ...
    if (count > kResultDisplayCap)
        s += QObject::tr("  (showing first %1)").arg(QLocale().toString((qulonglong)kResultDisplayCap));
    return s;
}
...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    setupUi();
    setupMenus();
    setWindowTitle(QObject::tr("Cheat Engine"));
    resize(760, 560);
    ...
    luaEngine_.setFilePicker([this](const std::string& setting) -> std::string {
        QString title = setting.empty() ? QObject::tr("Select file")
                                        : QObject::tr("Select file for %1").arg(QString::fromStdString(setting));
    ...
    connect(timer, &QTimer::timeout, this, [this]() {
        ...
        if (currentPid_ > 0 && ::kill(currentPid_, 0) != 0 && errno == ESRCH) {
            processLabel_->setText(QObject::tr("Process %1 has exited").arg(currentPid_));
            statusBar()->showMessage(QObject::tr("Target process exited"), 5000);
...
void MainWindow::setupMenus() {
    ...
    auto stub = [](QMenu* m, const QString& text) {
        ...
        a->setEnabled(false);
        a->setToolTip(QObject::tr("Not yet implemented"));
        return a;
    };

    // ── File ──
    auto* file = menuBar()->addMenu(QObject::tr("&File"));
    stub(file, QObject::tr("Add scan tab"))->setShortcut(QKeySequence("Ctrl+T"));
    file->addAction(QObject::tr("Clear list"), this, [this]() {
        resultsModel_->clear(); lastResult_.reset(); undoResult_.reset(); updateScanButtons();
    });
    file->addAction(QObject::tr("Open Process"), this, &MainWindow::onOpenProcess, QKeySequence("Ctrl+O"));
    stub(file, QObject::tr("Open File"))->setShortcut(QKeySequence("Ctrl+Shift+O"));
    { auto* a = stub(file, QObject::tr("Save File")); a->setShortcut(QKeySequence("Ctrl+Shift+S")); a->setVisible(false); }
    file->addSeparator();
    file->addAction(QObject::tr("Save"), this, &MainWindow::onSaveTable, QKeySequence("Ctrl+S"));
    file->addAction(QObject::tr("Save As..."), this, &MainWindow::onSaveTable);
    file->addAction(QObject::tr("Load"), this, &MainWindow::onLoadTable, QKeySequence("Ctrl+L"));
    recentMenu_ = file->addMenu(QObject::tr("Load Recent")); // populated from QSettings below
    recentMenu_->setToolTipsVisible(true);      // show the full path on hover
    rebuildRecentMenu();
    stub(file, QObject::tr("Sign table"));
    file->addSeparator();
    file->addAction(QObject::tr("Save current scanresults"), this, &MainWindow::onSaveScanResults,
                    QKeySequence("Alt+Shift+S"));
    stub(file, QObject::tr("Delete scanresult"));
    file->addSeparator();
    file->addAction(QObject::tr("Generate generic trainer lua script from table"), this, &MainWindow::onCreateTrainer);
    file->addSeparator();
    file->addAction(QObject::tr("Connect to ceserver..."), this, &MainWindow::onConnectCeserver);  // Linux extra
    file->addSeparator();
    file->addAction(QObject::tr("Quit"), this, &QWidget::close, QKeySequence("Ctrl+Q"));

    // ── Top-level menus in CE's order (D3D dropped — Windows-only, see below) ──
    // File · Edit · Process · Table · Tools · .Net · Network · Plugins ·
    // Languages · Help.
    auto* edit = menuBar()->addMenu(QObject::tr("&Edit"));
    auto* process = menuBar()->addMenu(QObject::tr("&Process"));
    auto* table = menuBar()->addMenu(QObject::tr("Table"));
    ...
    auto* tools = menuBar()->addMenu(QObject::tr("&Tools"));
    auto* dotnet = menuBar()->addMenu(".Net");
    // CE's .Net/Mono menu: dissect a Mono/IL2CPP runtime's classes and fields.
    dotnet->addAction(QObject::tr("Mono dissector..."), this, &MainWindow::openMonoDissector);
    auto* network = menuBar()->addMenu(QObject::tr("Network"));
    // CE also has Plugins and Languages menus, but this build has no plugin loader
    // and the UI language is picked from Settings (translations are shipped as
    // translations/cheatengine_<locale>.qm), so they are dropped rather than shown
    // as empty dropdowns (same call the D3D menu got above).

    // ── Table (CE: Lua script + forms) ──
    table->addAction(QObject::tr("Show Cheat Table Lua Script"), this, &MainWindow::showTableLuaScript,
                     QKeySequence("Ctrl+Alt+L"));
    table->addSeparator();
    table->addAction(QObject::tr("Create form"), this, [this]() {
        auto* fd = new FormDesigner(this); fd->setAttribute(Qt::WA_DeleteOnClose); fd->show();
    });
    stub(table, QObject::tr("Resynchronize forms with Lua"));
    table->addSeparator();
    stub(table, QObject::tr("Add file"));

    // (D3D menu intentionally omitted on Linux — see note at the menu creation above.)

    // ── .Net (Windows-only) ──
    stub(dotnet, QObject::tr("Get object list"));

    // ── Network (ceserver client options) ──
    stub(network, QObject::tr("Compression"));
    stub(network, QObject::tr("Scan changed regions only"));
    stub(network, QObject::tr("Scan paged (physical) memory only"));
    auto* netRead = network->addMenu(QObject::tr("Memory read method"));
    stub(netRead, "0: /proc/pid/mem"); stub(netRead, "1: ptrace peek"); stub(netRead, "2: process_vm_readv");
    auto* netWrite = network->addMenu(QObject::tr("Memory write method"));
    ...
    tools->addAction(QObject::tr("Memory Browser"), this, &MainWindow::onMemoryView, QKeySequence("Ctrl+M"));
    tools->addSeparator();
    ...
    tools->addSeparator();
    edit->addAction(QObject::tr("Settings..."), this, [this]() { openSettingsDialog(); });

    // ── Process menu ──
    process->addAction(QObject::tr("Open Process..."), this, &MainWindow::onOpenProcess);
    ...
    process->addAction(QObject::tr("Create Process..."), this, [this]() {
        auto path = QFileDialog::getOpenFileName(this, QObject::tr("Open file as process"), "", QObject::tr("Executables (*)"));
        ...
        if (pid < 0) { QMessageBox::warning(this, QObject::tr("Create Process"), QObject::tr("fork() failed.")); return; }
        usleep(120000);   // let it map its image
        currentPid_ = pid;
        ceserverClient_.reset();
        process_ = std::make_unique<os::LinuxProcessHandle>(pid);
        processLabel_->setText(QObject::tr("PID: %1 - %2 (created)").arg(pid).arg(QFileInfo(path).fileName()));
        setWindowTitle(QObject::tr("Cheat Engine - %1 (%2)").arg(QFileInfo(path).fileName()).arg(pid));
        addressListModel_->setProcess(process_.get());
        resultsModel_->setProcess(process_.get());
        firstScanBtn_->setEnabled(true);
        updateScanButtons();
    });
    process->addAction(QObject::tr("Process/System Info"), this, [this]() {
        if (!process_) { QMessageBox::information(this, QObject::tr("Process Info"), QObject::tr("No process opened.")); return; }
        ...
        QString info = QObject::tr(
            ...
            .arg(mods.size())
            .arg(threads.size());
        QMessageBox::information(this, QObject::tr("Process/System Info"), info);
    });
    // "Pause the process" toggle (CE's pause-the-game) — SIGSTOP/SIGCONT the target.
    auto* pauseAct = process->addAction(QObject::tr("Pause the process"));
    ...
    auto* pauseScanAct = process->addAction(QObject::tr("Pause target while scanning"));
    pauseScanAct->setCheckable(true);
    pauseScanAct->setToolTip(QObject::tr("Suspend the target during each First/Next Scan so its "
    ...
    // ── Table menu ──
    table->addAction(QObject::tr("Auto Assemble..."), this, [this]() {
    ...
    tools->addAction(QObject::tr("File Patcher..."), this, [this]() {
        auto* dlg = new FilePatcher(this);
        dlg->setAttribute(Qt::WA_DeleteOnClose);
        dlg->show();
    });
    // CE frmSaveMemoryRegionUnit (Save memoryregion): dump a From..To range to a file.
    tools->addAction(QObject::tr("Save Memory Region..."), this, [this]() {
        if (!process_) { QMessageBox::warning(this, QObject::tr("Save Memory Region"), QObject::tr("Open a process first.")); return; }
        auto* dlg = new QDialog(this);
        dlg->setWindowTitle(QObject::tr("Save memoryregion"));
        auto* v = new QVBoxLayout(dlg);
        v->addWidget(new QLabel(QObject::tr("Add the region of memory you want to save")));
        auto* form = new QFormLayout;
        auto* fromEdit = new QLineEdit("0");
        auto* toEdit = new QLineEdit("0");
        form->addRow(QObject::tr("From"), fromEdit);
        form->addRow(QObject::tr("To"), toEdit);
        v->addLayout(form);
        ...
        connect(buttons, &QDialogButtonBox::accepted, this, [this, dlg, fromEdit, toEdit]() {
            ...
            if (to <= from) { QMessageBox::warning(dlg, QObject::tr("Save"), QObject::tr("\"To\" must be greater than \"From\".")); return; }
            auto path = QFileDialog::getSaveFileName(dlg, QObject::tr("Save memory region"), "region.bin");
            if (path.isEmpty()) return;
            std::vector<uint8_t> buf(to - from);
            auto r = process_->read(from, buf.data(), buf.size());
            if (!r || *r == 0) { QMessageBox::warning(dlg, QObject::tr("Save"), QObject::tr("Could not read that region.")); return; }
            std::ofstream f(path.toStdString(), std::ios::binary);
            f.write(reinterpret_cast<const char*>(buf.data()), (std::streamsize)*r);
            QMessageBox::information(dlg, QObject::tr("Saved"), QObject::tr("Wrote %1 bytes to %2.").arg(*r).arg(path));
            dlg->accept();
        });
        dlg->setAttribute(Qt::WA_DeleteOnClose);
        dlg->show();
    ...
    tools->addAction(QObject::tr("Memory Allocations..."), this, [this]() {
        if (!process_) { QMessageBox::warning(this, QObject::tr("Memory Allocations"), QObject::tr("Open a process first.")); return; }
        auto* dlg = new QDialog(this);
        dlg->setWindowTitle(QObject::tr("Memory Allocations"));
        dlg->resize(360, 300);
        auto* v = new QVBoxLayout(dlg);
        ...
        auto refill = [this, list]() {
            ...
            for (const auto& a : allocations_)
                list->addItem(QObject::tr("0x%1  (%2 bytes)").arg(a.first, 0, 16).arg(a.second));
        };
        refill();
        v->addWidget(list);
        auto* row = new QHBoxLayout;
        auto* allocBtn = new QPushButton(QObject::tr("Allocate"));
        auto* freeBtn = new QPushButton(QObject::tr("Free"));
        ...
        connect(allocBtn, &QPushButton::clicked, this, [this, dlg, refill]() {
            bool ok = false;
            int size = QInputDialog::getInt(dlg, QObject::tr("Allocate"), QObject::tr("Size (bytes):"), 4096, 1, 1 << 30, 1, &ok);
            if (!ok) return;
            auto r = process_->allocate(size, ce::MemProt::All, 0);
            if (!r) { QMessageBox::warning(dlg, QObject::tr("Allocate"), QObject::tr("Allocation failed.")); return; }
    ...
    tools->addAction(QObject::tr("Generate Trainer (executable)..."), this, [this]() {
        auto path = QFileDialog::getSaveFileName(this, QObject::tr("Generate Trainer executable"), "trainer");
        if (path.isEmpty()) return;
        try {
            ...
            if (err.empty())
                statusBar()->showMessage(QObject::tr("Trainer executable written to ") + path, 6000);
            else
                QMessageBox::critical(this, QObject::tr("Generate Trainer"), QString::fromStdString(err));
        } catch (const std::exception& ex) {
            QMessageBox::critical(this, QObject::tr("Generate Trainer"), ex.what());
        }
    });
    // CE frmStringPointerScanUnit (Structure spider): find pointer paths to a target
    // and dissect the structure there.
    tools->addAction(QObject::tr("Structure Spider..."), this, [this]() {
        if (!process_) { QMessageBox::warning(this, QObject::tr("Structure Spider"), QObject::tr("Open a process first.")); return; }
        bool ok = false;
        QString ts = QInputDialog::getText(this, QObject::tr("Structure Spider"), QObject::tr("Target address (hex):"),
        ...
        auto* dlg = new QDialog(this);
        dlg->setWindowTitle(QObject::tr("Structure spider"));
        dlg->resize(520, 420);
        auto* v = new QVBoxLayout(dlg);
        v->addWidget(new QLabel(QObject::tr("%1 pointer path(s) to 0x%2:")
                                    .arg(paths.size()).arg((qulonglong)target, 0, 16)));
        auto* list = new QListWidget;
        for (const auto& p : paths) list->addItem(QString::fromStdString(p.toString()));
        v->addWidget(list);
        auto* row = new QHBoxLayout;
        auto* dissectBtn = new QPushButton(QObject::tr("Dissect structure at target"));
    ...
    tools->addAction(QObject::tr("Group Scan Generator..."), this, [this]() {
        if (!process_ || !lastResult_) { QMessageBox::warning(this, QObject::tr("Group Scan Generator"), QObject::tr("Run a scan and select results first.")); return; }
        auto sel = resultsView_->selectionModel()->selectedRows();
        if (sel.size() < 2) { QMessageBox::warning(this, QObject::tr("Group Scan Generator"), QObject::tr("Select at least two results.")); return; }
        ...
        auto* dlg = new QDialog(this);
        dlg->setWindowTitle(QObject::tr("Group Scan Generator"));
        auto* v = new QVBoxLayout(dlg);
        v->addWidget(new QLabel(QObject::tr("Grouped-scan pattern (offset:value, 4-byte):")));
        auto* edit = new QPlainTextEdit(pattern); edit->setReadOnly(true);
        v->addWidget(edit);
        auto* row = new QHBoxLayout;
        auto* copyBtn = new QPushButton(QObject::tr("Copy"));
    ...
    tools->addAction(QObject::tr("Save Disassembly..."), this, [this]() {
        if (!process_) { QMessageBox::warning(this, QObject::tr("Save Disassembly"), QObject::tr("Open a process first.")); return; }
        bool ok = false;
        QString fromS = QInputDialog::getText(this, QObject::tr("Save Disassembly"), QObject::tr("From (hex):"),
                                              QLineEdit::Normal, "0", &ok);
        if (!ok) return;
        QString toS = QInputDialog::getText(this, QObject::tr("Save Disassembly"), QObject::tr("To (hex):"),
                                            QLineEdit::Normal, "0", &ok);
        if (!ok) return;
        uintptr_t from = fromS.toULongLong(nullptr, 16), to = toS.toULongLong(nullptr, 16);
        if (to <= from || to - from > (64u << 20)) {
            QMessageBox::warning(this, QObject::tr("Save Disassembly"), QObject::tr("Invalid or too-large range.")); return;
        }
        std::vector<uint8_t> buf(to - from);
        auto r = process_->read(from, buf.data(), buf.size());
        if (!r || *r == 0) { QMessageBox::warning(this, QObject::tr("Save Disassembly"), QObject::tr("Could not read that range.")); return; }
        ...
        auto path = QFileDialog::getSaveFileName(this, QObject::tr("Save disassembly"), "disasm.asm");
        if (path.isEmpty()) return;
        std::ofstream f(path.toStdString());
        for (const auto& i : insns)
            f << std::hex << i.address << ": " << i.mnemonic
              << (i.operands.empty() ? "" : " ") << i.operands << "\n";
        QMessageBox::information(this, QObject::tr("Saved"), QObject::tr("Wrote %1 instructions to %2.")
                                    .arg(insns.size()).arg(path));
    });
    // CE frmMemoryViewExUnit (Graphical Memory View): bytes as pixels.
    process->addAction(QObject::tr("Wait for process..."), this, [this]() {
        bool ok = false;
        auto name = QInputDialog::getText(this, QObject::tr("Wait for process"),
            QObject::tr("Process name substring (will auto-attach when seen):"),
            QLineEdit::Normal, QObject::tr(""), &ok);
        if (!ok || name.isEmpty()) return;
        if (processWatcher_) processWatcher_->stop();
        processWatcher_ = std::make_unique<os::ProcessWatcher>();
        auto* status = statusBar();
        status->showMessage(QObject::tr("Watching for '%1'…").arg(name));
        processWatcher_->start(name.toStdString(),
            [this, status](pid_t pid, const std::string& procName) {
                QMetaObject::invokeMethod(this, [this, pid, procName, status]() {
                    status->showMessage(
                        QObject::tr("Auto-attached to %1 (pid %2)")
                            .arg(QString::fromStdString(procName)).arg(pid),
                        4000);
                    currentPid_ = pid;
                    ceserverClient_.reset();
                    process_ = std::make_unique<os::LinuxProcessHandle>(pid);
                    processLabel_->setText(QObject::tr("PID: %1 - %2 (auto-attached)")
                        .arg(pid).arg(QString::fromStdString(procName)));
                    setWindowTitle(QObject::tr("Cheat Engine - %1 (%2)")
                        .arg(QString::fromStdString(procName)).arg(pid));
    ...
    auto* snapshotMenu = tools->addMenu(QObject::tr("Snapshot"));
    snapshotMenu->addAction(QObject::tr("Capture writable regions"), this, [this]() {
        if (!process_) {
            QMessageBox::warning(this, QObject::tr("No process"), QObject::tr("Open a process before capturing a snapshot."));
            return;
        }
        snapshot_ = std::make_unique<Snapshot>(Snapshot::capture(*process_));
        QMessageBox::information(this, QObject::tr("Snapshot captured"),
            QObject::tr("Captured %1 region(s), %2 bytes total.")
                .arg(snapshot_->regionCount()).arg(snapshot_->byteCount()));
    });
    snapshotMenu->addAction(QObject::tr("Diff against current"), this, [this]() {
        if (!process_ || !snapshot_) {
            QMessageBox::warning(this, QObject::tr("Need both"), QObject::tr("Capture a baseline snapshot first, then open a process."));
            return;
        }
        Snapshot now = Snapshot::capture(*process_);
        auto diffs = snapshot_->diff(now);
        if (diffs.empty()) {
            QMessageBox::information(this, QObject::tr("Diff"), QObject::tr("No byte-level differences detected."));
            return;
        }
        // Show the first 200 diffs in a simple dialog; full list can be saved.
        QString text;
        text += QObject::tr("%1 byte%2 changed since the baseline.\n\n")
        ...
        auto* dlg = new QDialog(this);
        dlg->setWindowTitle(QObject::tr("Snapshot diff"));
        dlg->resize(640, 480);
    ...
    snapshotMenu->addAction(QObject::tr("Restore baseline"), this, [this]() {
        if (!process_ || !snapshot_) {
            QMessageBox::warning(this, QObject::tr("Need both"), QObject::tr("Capture a baseline snapshot first."));
            return;
        }
        auto answer = QMessageBox::question(this, QObject::tr("Restore?"),
            QObject::tr("Write the baseline back into the process? This overwrites every "
                    "writable region captured (%1 regions, %2 bytes).")
                .arg(snapshot_->regionCount()).arg(snapshot_->byteCount()));
        if (answer != QMessageBox::Yes) return;
        auto written = snapshot_->restore(*process_);
        QMessageBox::information(this, QObject::tr("Restored"),
            QObject::tr("Wrote %1 bytes back into the process.").arg(written));
    });
    snapshotMenu->addAction(QObject::tr("Save to file..."), this, [this]() {
        if (!snapshot_) {
            QMessageBox::warning(this, QObject::tr("No snapshot"), QObject::tr("Capture a snapshot first."));
            return;
        }
        auto path = QFileDialog::getSaveFileName(this, QObject::tr("Save snapshot"), "snapshot.cesnap",
            QObject::tr("Snapshot (*.cesnap);;All (*)"));
        if (path.isEmpty()) return;
        if (!snapshot_->save(path.toStdString())) {
            QMessageBox::warning(this, QObject::tr("Save failed"), QObject::tr("Could not write ") + path);
        }
    });
    snapshotMenu->addAction(QObject::tr("Load from file..."), this, [this]() {
        auto path = QFileDialog::getOpenFileName(this, QObject::tr("Load snapshot"), "",
            QObject::tr("Snapshot (*.cesnap);;All (*)"));
        if (path.isEmpty()) return;
        auto loaded = std::make_unique<Snapshot>();
        std::string err;
        if (!loaded->load(path.toStdString(), &err)) {
            QMessageBox::warning(this, QObject::tr("Load failed"), QString::fromStdString(err));
            return;
        }
        snapshot_ = std::move(loaded);
        QMessageBox::information(this, QObject::tr("Snapshot loaded"),
            QObject::tr("Loaded %1 region(s), %2 bytes.")
                .arg(snapshot_->regionCount()).arg(snapshot_->byteCount()));
    });
    tools->addSeparator();
    tools->addSeparator();
    tools->addAction(QObject::tr("Speedhack..."), this, [this]() {
        auto* dlg = new QDialog(this);
        dlg->setWindowTitle(QObject::tr("Speedhack"));
        dlg->resize(300, 120);
        auto* layout = new QVBoxLayout(dlg);
        auto* label = new QLabel(QObject::tr("Speed: 1.0x"));
        auto* slider = new QSlider(Qt::Horizontal);
        slider->setRange(1, 100);  // 0.1x to 10.0x
        slider->setValue(10);      // 1.0x default
        connect(slider, &QSlider::valueChanged, [label](int v) {
            double speed = v / 10.0;
            label->setText(QObject::tr("Speed: %1x").arg(speed, 0, 'f', 1));
        });
        auto* applyBtn = new QPushButton(QObject::tr("Apply (inject speedhack + set speed)"));
        connect(applyBtn, &QPushButton::clicked, [this, slider, label]() {
            ...
            if (process_) {
                ...
                if (!loaded) {
                    ...
                    if (!r) {
                        label->setText(QObject::tr("Inject failed: %1")
                                           .arg(QString::fromStdString(r.error())));
                        return;  // don't set a speed the plugin can't read
                    }
                }
                label->setText(QObject::tr("Active, Speed: %1x").arg(speed, 0, 'f', 1));
            }
    ...
    // ── Help (CE verbatim) ──
    auto* help = menuBar()->addMenu(QObject::tr("&Help"));
    help->addAction(QObject::tr("Cheat Engine Help"), this, [this]() {
        openHelpDoc("README.md", QObject::tr("Cheat Engine for Linux: Help"),
                    "https://github.com/wleeaf/cheat-engine-linux#readme");
    });
    help->addAction(QObject::tr("Lua documentation"), this, [this]() {
        openHelpDoc("docs/SCRIPTING.md", QObject::tr("Lua / Scripting documentation"),
                    "https://github.com/wleeaf/cheat-engine-linux/blob/main/docs/SCRIPTING.md");
    });
    stub(help, QObject::tr("Cheat Engine Tutorial"));
    stub(help, QObject::tr("Cheat Engine Tutorial (x86_64)"));
    { auto* a = stub(help, QObject::tr("Cheat Engine Tutorial (AArch64)")); a->setVisible(false); }
    stub(help, QObject::tr("Cheat Engine Tutorial Games"));
    { auto* a = stub(help, QObject::tr("Generate errorlogs")); a->setVisible(false); }
    { auto* a = stub(help, QObject::tr("Test access violation")); a->setVisible(false); }
    { auto* a = stub(help, QObject::tr("Test access violation in thread")); a->setVisible(false); }
    help->addSeparator();
    help->addAction(QObject::tr("About"), this, [this]() {
        QMessageBox::about(this, QObject::tr("Cheat Engine for Linux"),
            QObject::tr("<h2>Cheat Engine for Linux</h2>"
...
void MainWindow::openHelpDoc(const QString& relPath, const QString& title, const QString& url) {
    ...
    v->addWidget(view);
    auto* row = new QHBoxLayout;
    auto* online = new QPushButton(QObject::tr("Open on GitHub"));
...
void MainWindow::showComments() {
    auto* dlg = new QDialog(this);
    dlg->setWindowTitle(QObject::tr("Comments"));
    dlg->resize(500, 400);
    auto* v = new QVBoxLayout(dlg);
    auto* tabs = new QTabWidget;                       // CE PageControl1
    auto* memo = new QPlainTextEdit;                   // CE Memo1
    memo->setPlainText(tableComment_);
    tabs->addTab(memo, QObject::tr("Comments"));                    // CE tsComment
    v->addWidget(tabs);
    auto* row = new QHBoxLayout;                        // CE Panel1
    row->addStretch();
    auto* closeBtn = new QPushButton(QObject::tr("Close"));         // CE Button1
    row->addWidget(closeBtn);
    v->addLayout(row);
...
void MainWindow::showTableLuaScript() {
    auto* dlg = new QDialog(this);
    dlg->setWindowTitle(QObject::tr("Cheat Table Lua Script"));
    dlg->resize(640, 460);
    auto* v = new QVBoxLayout(dlg);
    auto* memo = new QPlainTextEdit;
    memo->setFont(settingsMonospaceFont());
    memo->setPlainText(tableLuaScript_);
    memo->setPlaceholderText(QObject::tr("-- Lua that runs when this table is opened; saved with the table."));
    v->addWidget(memo);

    auto* row = new QHBoxLayout;
    auto* execBtn = new QPushButton(QObject::tr("Execute script"));
    ...
    connect(execBtn, &QPushButton::clicked, this, [this, memo]() {
        ...
        statusBar()->showMessage(
            res.has_value() ? QObject::tr("Table Lua executed.")
                            : QObject::tr("Table Lua error: %1").arg(QString::fromStdString(res.error())),
            8000);
    });
    connect(closeBtn, &QPushButton::clicked, dlg, &QDialog::accept);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
...
void MainWindow::showOverlayDialog() {
    QDialog dialog(this);
    dialog.setWindowTitle(QObject::tr("Overlay"));
    auto* layout = new QVBoxLayout(&dialog);
    auto* osdCheck = new QCheckBox(QObject::tr("OSD text"));
    auto* crosshairCheck = new QCheckBox(QObject::tr("Crosshair"));
    auto* showButton = new QPushButton(QObject::tr("Show Overlay"));
    auto* hideButton = new QPushButton(QObject::tr("Hide Overlay"));
    auto* closeButton = new QPushButton(QObject::tr("Close"));
...
void MainWindow::updateOverlayStatus() {
    ...
    overlayWindow_->setStatusText(QObject::tr("%1 | Records %2/%3 active")
        .arg(processLabel_->text())
        .arg(active)
        .arg(total));
}
...
void MainWindow::setupUi() {
    ...
    openBtn->setToolTip(QObject::tr("Open process"));
    ...
    processLabel_ = new QLabel(QObject::tr("No process selected"));
    ...
    connect(addSelSc, &QShortcut::activated, this, [this]() {
        ...
        for (auto& idx : sel)
            addressListModel_->addEntry(resultsModel_->addressAt(idx.row()), lastResultType_,
                                        QObject::tr("No description"), "", lastResultValueSize_);
    });
    ...
    connect(resultsView_, &QTableView::customContextMenuRequested, this, [this](const QPoint& pos) {
        ...
        auto* addAct = menu.addAction(sel.size() > 1
            ? QObject::tr("Add %1 selected addresses to the addresslist").arg(sel.size())
            : QObject::tr("Add selected addresses to the addresslist"));
        auto* browseAct = menu.addAction(QObject::tr("Browse this memory region"));
        auto* disasmAct = menu.addAction(QObject::tr("Disassemble this memory region"));
        auto* copyAct = menu.addAction(QObject::tr("Copy selected addresses"));
        auto* removeAct = menu.addAction(sel.size() > 1
            ? QObject::tr("Remove %1 selected addresses from the list").arg(sel.size())
            : QObject::tr("Remove selected address from the list"));
        menu.addSeparator();
        auto* accAct = menu.addAction(QObject::tr("Find out what accesses this address"));
        auto* wrAct  = menu.addAction(QObject::tr("Find out what writes to this address"));
    ...
    foundLabel_ = new QLabel(QObject::tr("Found: 0"));
    foundRow->addWidget(foundLabel_);
    foundRow->addStretch();
    auto* resHexCheck = new QCheckBox(QObject::tr("Hex"));
    resHexCheck->setToolTip(QObject::tr("Show result values in hexadecimal"));
    ...
    auto* memViewBtn = new QPushButton(QObject::tr("Memory View"));
    connect(memViewBtn, &QPushButton::clicked, this, &MainWindow::onMemoryView);
    auto* addAddrBtn = new QPushButton(QObject::tr("Add Address"));
    connect(addAddrBtn, &QPushButton::clicked, this, [this]() {
        ...
        dlg.setWindowTitle(QObject::tr("Add address"));
        ...
        if (addr) {
            ...
            addressListModel_->addEntry(*addr, dlg.valueType(), QObject::tr("Manual entry"), expr);
        } else
            QMessageBox::warning(this, QObject::tr("Add Address"), QObject::tr("Could not resolve \"%1\".").arg(text.trimmed()));
    });
    ...
    valueLayout->addWidget(new QLabel(QObject::tr("Value:")));
    ...
    hexCheck_ = new QCheckBox(QObject::tr("Hex"));
    hexCheck_->setToolTip(QObject::tr("Interpret integer scan values as hexadecimal"));
    valueLayout->addWidget(hexCheck_);
    rightLayout->addLayout(valueLayout);

    // Scan type
    scanTypeCombo_ = new QComboBox;
    scanTypeCombo_->addItems({QObject::tr("Exact Value"), QObject::tr("Bigger than..."), QObject::tr("Smaller than..."),
        QObject::tr("Value between..."), QObject::tr("Unknown initial value"), QObject::tr("Increased value"),
        QObject::tr("Decreased value"), QObject::tr("Changed value"), QObject::tr("Unchanged value"), QObject::tr("Same as first scan"),
        QObject::tr("Increased value by..."), QObject::tr("Decreased value by...")});
    ...
    floatRoundingCombo_->addItems({QObject::tr("Exact"), QObject::tr("Rounded"), QObject::tr("Truncated"), QObject::tr("Extreme")});
    floatRoundingCombo_->setCurrentIndex(1);  // CE defaults float scans to Rounded
    floatToleranceEdit_ = new QLineEdit;
    floatToleranceEdit_->setPlaceholderText(QObject::tr("Tolerance"));
    ...
    auto updateFloatOptions = [this]() {
        ...
        QString ph = QObject::tr("e.g. 100");
        switch (vt) {
            case ValueType::Float:
            case ValueType::Double:        ph = QObject::tr("e.g. 3.14"); break;
            case ValueType::String:
            case ValueType::UnicodeString: ph = QObject::tr("text to find"); break;
            case ValueType::ByteArray:     ph = QObject::tr("e.g. 48 8B ?? 05"); break;
            case ValueType::Binary:        ph = QObject::tr("e.g. 0110??01"); break;
            case ValueType::Pointer:       ph = QObject::tr("e.g. 0x7fabc000"); break;
            default:                       ph = QObject::tr("e.g. 100"); break;
        }
    ...
    firstScanBtn_ = new QPushButton(QObject::tr("First Scan"));
    // Accent "primary action" styling comes from the theme (QPushButton#primaryButton).
    firstScanBtn_->setObjectName("primaryButton");
    nextScanBtn_ = new QPushButton(QObject::tr("Next Scan"));
    nextScanBtn_->setEnabled(false);
    undoScanBtn_ = new QPushButton(QObject::tr("Undo Scan"));
    ...
    // Scan options group
    auto* optGroup = new QGroupBox(QObject::tr("Memory Scan Options"));
    auto* optLayout = new QGridLayout(optGroup);
    optLayout->addWidget(new QLabel(QObject::tr("From:")), 0, 0);
    ...
    optLayout->addWidget(new QLabel(QObject::tr("To:")), 1, 0);
    ...
    writableCheck_ = new QCheckBox(QObject::tr("Writable"));
    writableCheck_->setTristate(true);
    writableCheck_->setCheckState(Qt::Checked);
    writableCheck_->setToolTip(QObject::tr("Checked: only writable regions. Unchecked: only "
                               "non-writable. Grey: any (don't care)."));
    optLayout->addWidget(writableCheck_, 2, 0, 1, 2);
    executableCheck_ = new QCheckBox(QObject::tr("Executable"));
    executableCheck_->setTristate(true);
    executableCheck_->setCheckState(Qt::PartiallyChecked);
    executableCheck_->setToolTip(QObject::tr("Checked: only executable regions. Unchecked: only "
                                 "non-executable. Grey: any (don't care)."));
    optLayout->addWidget(executableCheck_, 3, 0, 1, 2);
    fastScanCheck_ = new QCheckBox(QObject::tr("Fast Scan"));
    ...
    percentCheck_ = new QCheckBox(QObject::tr("Compare by %"));
    ...
    auto* percent2Label = new QLabel(QObject::tr("Percent max:"));
    ...
    caseSensitiveCheck_ = new QCheckBox(QObject::tr("Case sensitive (text)"));
    ...
    // Mapped = other file-backed maps. Backed by ScanConfig::scan{Private,Image,Mapped}.
    {
        ...
        memPrivateCheck_ = new QCheckBox(QObject::tr("Private"));
        memPrivateCheck_->setToolTip(QObject::tr("Scan MEM_PRIVATE regions (anonymous heap, stack, private maps)"));
        memPrivateCheck_->setChecked(s.value("scan/memPrivate", true).toBool());
        memImageCheck_ = new QCheckBox(QObject::tr("Image"));
        memImageCheck_->setToolTip(QObject::tr("Scan MEM_IMAGE regions (loaded executable modules)"));
        memImageCheck_->setChecked(s.value("scan/memImage", true).toBool());
        memMappedCheck_ = new QCheckBox(QObject::tr("Mapped"));
        memMappedCheck_->setToolTip(QObject::tr("Scan MEM_MAPPED regions (other file-backed maps)"));
    ...
    optLayout->addWidget(new QLabel(QObject::tr("Text encoding:")), 7, 0);
    ...
    connect(addressListModel_, &AddressListModel::valueReverted, this,
        [this](uintptr_t addr, const QString& wrote, const QString& now) {
            statusBar()->showMessage(
                QObject::tr("Value at 0x%1 was reverted (wrote %2, now %3). It looks protected: "
    ...
    connect(addressListView_, &QWidget::customContextMenuRequested, this, [this](const QPoint& pos) {
        ...
        if (!selected.isEmpty()) {
            ...
            if (selected.size() == 1 && addressListModel_->isScriptEntry(firstRow)) {
                menu.addAction(QObject::tr("Edit script..."), [this, firstRow]() { editScriptEntry(firstRow); });
                menu.addSeparator();
                ...
                menu.addAction(QObject::tr("Change address..."), [this, firstRow]() {
            ...
            menu.addAction(QObject::tr("Cut"), this, [this]() { onCopyAddresses(); onDeleteAddresses(); }, QKeySequence::Cut);
            menu.addAction(QObject::tr("Copy"), this, &MainWindow::onCopyAddresses, QKeySequence::Copy);
            menu.addAction(QObject::tr("Paste"), this, [this]() {
            ...
            if (firstRow >= 0 && firstRow < (int)ents0.size()) {
                ...
                auto* copyMenu = menu.addMenu(QObject::tr("Copy field"));
                copyMenu->addAction(QObject::tr("Description"), [fe]() {
                    QApplication::clipboard()->setText(fe.description);
                });
                if (!fe.isGroup && fe.autoAsmScript.isEmpty()) {
                    copyMenu->addAction(QObject::tr("Address"), [fe]() {
                        QApplication::clipboard()->setText(
                            fe.addressExpr.isEmpty() ? QString("0x%1").arg(fe.address, 0, 16)
                                                     : fe.addressExpr);
                    });
                    copyMenu->addAction(QObject::tr("Value"), [fe]() {
                        QApplication::clipboard()->setText(fe.currentValue);
                    });
                }
                if (!fe.autoAsmScript.isEmpty())
                    copyMenu->addAction(QObject::tr("Script"), [fe]() {
                        QApplication::clipboard()->setText(fe.autoAsmScript);
                    });
            }
            menu.addAction(QObject::tr("Delete"), this, &MainWindow::onDeleteAddresses, QKeySequence::Delete);

            menu.addSeparator();
            menu.addAction(QObject::tr("Indent"), [this, selected]() {
                QList<int> rows;
                for (const auto& idx : selected) rows.append(idx.row());
                addressListModel_->indentRows(rows);
            });
            menu.addAction(QObject::tr("Outdent"), [this, selected]() {
                QList<int> rows;
                for (const auto& idx : selected) rows.append(idx.row());
            ...
            menu.addAction(QObject::tr("Add to new group..."), [this, selected]() {
                QList<int> rows;
                for (const auto& idx : selected) rows.append(idx.row());
                if (rows.isEmpty()) return;
                bool ok = false;
                QString name = QInputDialog::getText(this, QObject::tr("New group"),
                    QObject::tr("Group name:"), QLineEdit::Normal, QObject::tr("-- Group --"), &ok);
                if (!ok) return;
            ...
            if (selected.size() == 1) {
                ...
                auto* up = menu.addAction(QObject::tr("Move Up"), [this, row]() { moveSelectedEntry(row, -1); });
                up->setShortcut(QKeySequence("Ctrl+Up"));
                auto* down = menu.addAction(QObject::tr("Move Down"), [this, row]() { moveSelectedEntry(row, +1); });
                down->setShortcut(QKeySequence("Ctrl+Down"));
            }

            menu.addSeparator();
            // Set the value of ALL selected entries at once (CE's batch set value).
            menu.addAction(QObject::tr("Set value..."), [this, selected]() {
                ...
                bool ok = false;
                QString v = QInputDialog::getText(this, QObject::tr("Set value"),
                    QObject::tr("New value for %1 selected entr%2:")
            ...
            menu.addAction(QObject::tr("Set value codec..."), [this, selected]() {
                if (selected.isEmpty()) return;
                QString cur = QString::fromStdString(
                    addressListModel_->entryCodecSpec(selected.first().row()));
                bool ok = false;
                QString spec = QInputDialog::getText(this, QObject::tr("Value codec"),
                    QObject::tr("Obfuscation codec for the stored value:\n"
                ...
                if (!spec.isEmpty() && spec.compare("none", Qt::CaseInsensitive) != 0) {
                    ...
                    if (!c) {
                        QMessageBox::warning(this, QObject::tr("Invalid codec"),
                            QObject::tr("Use none, xor:0xKEY, add:N, rol:N, or ror:N."));
                        return;
                    }
                    codec = *c;
                }
            ...
            auto* beAction = menu.addAction(QObject::tr("Big-endian value"));
            ...
            menu.addSeparator();
            auto* typeMenu = menu.addMenu(QObject::tr("Change type"));
            ...
            auto* hexAct = menu.addAction(QObject::tr("Show as hexadecimal"));
            ...
            if (process_) {
                menu.addAction(QObject::tr("Convert to module+offset"), [this, selected]() {
                    if (!process_) return;
                    ...
                    statusBar()->showMessage(
                        QObject::tr("Converted %1 address%2 to module+offset%3")
                            .arg(converted).arg(converted == 1 ? "" : "es")
                            .arg(skipped ? QObject::tr(" (%1 skipped)").arg(skipped) : QString()), 4000);
                });
            }
            ...
            if (firstRow >= 0 && firstRow < (int)ents0.size()
                ...
                menu.addAction(QObject::tr("Browse this memory region"), [this, selected]() {
            ...
            menu.addAction(QObject::tr("Find what accesses this address"), [this, selected]() {
                if (!selected.isEmpty())
                    startCodeFinder(selected.first().row(), false);
            });
            menu.addAction(QObject::tr("Find what writes to this address"), [this, selected]() {
                if (!selected.isEmpty())
            ...
            if (firstRow >= 0 && firstRow < (int)ents0.size()
                && !ents0[firstRow].isGroup && ents0[firstRow].autoAsmScript.isEmpty()) {
                menu.addAction(QObject::tr("Pointer scan for this address"), [this, selected]() {
                    if (!process_ || selected.isEmpty()) return;
            ...
            menu.addAction(QObject::tr("Change Color..."), [this, selected]() {
                const auto& entries = addressListModel_->entries();
                if (selected.isEmpty()) return;
                int row = selected.first().row();
                QColor initial = (row < (int)entries.size() && !entries[row].color.isEmpty())
                    ? QColor(entries[row].color) : QColor(Qt::black);
                QColor c = QColorDialog::getColor(initial, this, QObject::tr("Change record color"));
                if (!c.isValid()) return;
                for (const auto& idx : selected)
            ...
            if (selected.size() == 1) {
                menu.addAction(QObject::tr("Set/Change dropdown selection options..."), [this, firstRow]() {
                    const auto& entries = addressListModel_->entries();
                    if (firstRow >= (int)entries.size()) return;
                    bool ok = false;
                    QString v = QInputDialog::getMultiLineText(this, QObject::tr("Dropdown options"),
                        QObject::tr("One \"value:label\" per line (or value alone):"),
            ...
            if (selected.size() == 1) {
                menu.addSeparator();
                menu.addAction(QObject::tr("Configure Hotkey..."), [this, selected]() {
                    int row = selected.first().row();
                    const auto& entries = addressListModel_->entries();
                    if (row < 0 || row >= (int)entries.size()) return;

                    QDialog dialog(this);
                    dialog.setWindowTitle(QObject::tr("Configure Hotkey"));
                    auto* layout = new QVBoxLayout(&dialog);
                    ...
                    auto* clearButton = buttons->addButton(QObject::tr("Clear"), QDialogButtonBox::ResetRole);
                ...
                menu.addAction(QObject::tr("Configure Value Hotkeys..."), [this, selected]() {
                    int row = selected.first().row();
                    const auto& entries = addressListModel_->entries();
                    if (row < 0 || row >= (int)entries.size()) return;

                    QDialog dialog(this);
                    dialog.setWindowTitle(QObject::tr("Configure Value Hotkeys"));
                    ...
                    setValueEdit->setPlaceholderText(QObject::tr("value to set"));
                    auto* buttons = new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel, &dialog);

                    layout->addWidget(label, 0, 0, 1, 2);
                    layout->addWidget(new QLabel(QObject::tr("Increase"), &dialog), 1, 0);
                    layout->addWidget(increaseEditor, 1, 1);
                    layout->addWidget(new QLabel(QObject::tr("Decrease"), &dialog), 2, 0);
                    layout->addWidget(decreaseEditor, 2, 1);
                    layout->addWidget(new QLabel(QObject::tr("Step"), &dialog), 3, 0);
                    layout->addWidget(stepEdit, 3, 1);
                    layout->addWidget(new QLabel(QObject::tr("Set value hotkey"), &dialog), 4, 0);
                    layout->addWidget(setEditor, 4, 1);
                    layout->addWidget(new QLabel(QObject::tr("Set to value"), &dialog), 5, 0);
        ...
        menu.addSeparator();
        menu.addAction(QObject::tr("Add Address Manually..."), [this]() {
            ...
            if (numeric && !a.contains('[') && !a.contains('+') && !a.contains(' ')) {
                id = addressListModel_->addEntry((uintptr_t)v, dlg.valueType(),
                                                 QObject::tr("No description"), QString(), (size_t)dlg.length());
            } else {
                ...
                id = addressListModel_->addEntry(addr0, dlg.valueType(),
                                                 QObject::tr("No description"), a, (size_t)dlg.length());
            }
            addressListModel_->setHexView(id, dlg.showHex());
            addressListModel_->setSigned(id, dlg.isSigned());
        });
        menu.addAction(QObject::tr("Add Group"), [this]() {
            addressListModel_->addGroup();
        });
        menu.addAction(QObject::tr("Paste"), this, &MainWindow::onPasteAddresses, QKeySequence::Paste);

        menu.addSeparator();
        menu.addAction(QObject::tr("Enable all"), [this]() { addressListModel_->setAllActive(true); });
        menu.addAction(QObject::tr("Disable all"), [this]() { addressListModel_->setAllActive(false); });
    ...
    scanForm->addWidget(new QLabel(QObject::tr("Scan Value")), 0, 0, Qt::AlignRight | Qt::AlignVCenter);
    ...
    scanForm->addWidget(new QLabel(QObject::tr("Scan Type")), 1, 0, Qt::AlignRight | Qt::AlignVCenter);
    scanForm->addWidget(scanTypeCombo_, 1, 1);
    scanForm->addWidget(new QLabel(QObject::tr("Value Type")), 2, 0, Qt::AlignRight | Qt::AlignVCenter);
    ...
    auto* advBtn = new QPushButton(QObject::tr("Advanced Options"));
    ...
    auto* extrasBtn = new QPushButton(QObject::tr("Table Extras"));
...
void MainWindow::attachToPid(pid_t pid, const QString& name) {
    ...
    processLabel_->setText(QObject::tr("PID: %1 - %2").arg(pid).arg(label));
    setWindowTitle(QObject::tr("Cheat Engine - %1 (%2)").arg(label).arg(pid));
    ...
    // explicit note, not a silent surprise later. See docs/CHALLENGING_TARGETS.md.
    {
        ...
        statusBar()->showMessage(
            QObject::tr("Attached to %1: %2").arg(label, QString::fromStdString(prof.summary())), 8000);
        ...
        if (prof.tracerPid != 0 && !prof.notes.empty())
            QMessageBox::warning(this, QObject::tr("Target already being traced"),
...
void MainWindow::onConnectCeserver() {
    ...
    pidEdit->setPlaceholderText(QObject::tr("Remote PID"));
    form->addRow(QObject::tr("PID:"), pidEdit);
    ...
    auto* connectBtn = new QPushButton(QObject::tr("Connect"));
    ...
    connect(connectBtn, &QPushButton::clicked, this, [&]() {
        ...
        if (!ok || pid <= 0) {
            statusLabel->setText(QObject::tr("Enter a valid remote PID."));
            return;
        }
        ...
        if (!client->connectTcp(hostEdit->text().toStdString(),
            ...
            statusLabel->setText(QObject::tr("Connect failed: %1").arg(QString::fromStdString(err)));
            return;
        }
        auto version = client->getVersion();
        if (!version) {
            statusLabel->setText(QObject::tr("Handshake failed: %1").arg(QString::fromStdString(version.error())));
            return;
        }
        auto handle = ce::os::RemoteProcessHandle::open(*client, pid);
        if (!handle) {
            statusLabel->setText(QObject::tr("Server refused PID %1").arg((int)pid));
            return;
        }
        ...
        currentPid_ = pid;
        processLabel_->setText(QObject::tr("REMOTE PID: %1 @ %2:%3 (%4)")
...
void MainWindow::onFirstScan() {
    ...
    if (lastResult_ != nullptr) {
        ...
        lastResult_.reset();
        undoResult_.reset();
        foundLabel_->setText(QObject::tr("Found: 0"));
        statusBar()->showMessage(QObject::tr("New scan: pick a value type and value, then First Scan."), 4000);
        updateScanButtons();
        return;
    }
    if (!process_) {
        statusBar()->showMessage(QObject::tr("No process attached; open one first (Ctrl+O) to scan."), 4000);
        return;
    }
    ...
    if (!fromExpr || !toExpr) {
        QMessageBox::warning(this, QObject::tr("Scan range"),
            QObject::tr("From/To must be a hex address, symbol, or module+offset (e.g. target2, target2+0x5000)."));
        return;
    }
    ...
    if (config.startAddress > config.stopAddress) {
        QMessageBox::warning(this, QObject::tr("Scan range"),
            QObject::tr("The From address must not be greater than the To address."));
        return;
    }
    ...
    if (config.valueType == ValueType::String || config.valueType == ValueType::UnicodeString) {
        ...
        if (!config.parseAOB(text.toStdString())) {
            QMessageBox::warning(this, QObject::tr("Array of byte scan"), QObject::tr("Enter a valid AOB pattern (e.g. \"7F 45 ?? 46\")."));
            return;
        }
        config.alignment = 1;
        ...
        if (config.byteArray.empty()) {
            QMessageBox::warning(this, QObject::tr("Binary scan"), QObject::tr("Enter a valid binary pattern (e.g. \"0110??01\")."));
            return;
        }
        config.alignment = 1;
        ...
        if (!config.parseGrouped(text.toStdString(), &error)) {
            QMessageBox::warning(this, QObject::tr("Grouped scan"), QString::fromStdString(error));
            return;
        }
        config.alignment = 1;
    ...
    try {
        ...
        firstScanBtn_->setEnabled(true);
        QMessageBox::warning(this, QObject::tr("Scan failed"),
            QObject::tr("The scan could not complete: %1\n\n"
    ...
    if (result->hasWriteError())
        QMessageBox::warning(this, QObject::tr("Scan results truncated"),
            QObject::tr("A scan-result file could not be fully written (the disk may be full). "
...
void MainWindow::onNextScan() {
    ...
    if (!lastResult_) {
        statusBar()->showMessage(QObject::tr("Run a First Scan before a Next Scan."), 4000);
        return;
    }
    ...
    try {
        ...
        QMessageBox::warning(this, QObject::tr("Scan failed"),
            QObject::tr("The next scan could not complete: %1").arg(e.what()));
        return;
    }
    nextScanBtn_->setEnabled(true);
...
void MainWindow::onResultDoubleClicked(const QModelIndex& index) {
    ...
    addressListModel_->addEntry(addr, lastResultType_, QObject::tr("No description"), "", lastResultValueSize_);
}
...
void MainWindow::editScriptEntry(int row) {
    ...
    auto* editor = new ScriptEditor(process_.get(), &autoAsm_, this);
    editor->setAttribute(Qt::WA_DeleteOnClose);
    editor->setWindowTitle(QObject::tr("Auto Assembler: ") + desc);
    editor->setDefaultDescription(desc);
    editor->setTableButtonText(QObject::tr("Save to Table"));
    editor->setScript(script.toStdString());
...
void MainWindow::onCreateTrainer() {
    auto path = QFileDialog::getSaveFileName(this, QObject::tr("Create Trainer (C source)"),
        "trainer.c", QObject::tr("C source (*.c);;All Files (*)"));
    if (path.isEmpty()) return;
    try {
        ce::CheatTable table = buildCheatTable();
        ce::TrainerGenerator gen;
        ...
        if (!f.open(QIODevice::WriteOnly)) {
            QMessageBox::warning(this, QObject::tr("Create Trainer"), QObject::tr("Could not write the trainer source file."));
            return;
        }
        f.write(src.data(), (qint64)src.size());
        statusBar()->showMessage(
            QObject::tr("Trainer C source written to %1 (compile with gcc)").arg(path), 6000);
    } catch (const std::exception& ex) {
        QMessageBox::critical(this, QObject::tr("Create Trainer"),
            QObject::tr("Trainer generation failed:\n%1").arg(ex.what()));
    }
}
...
void MainWindow::onSaveScanResults() {
    ...
    if (rows == 0) {
        statusBar()->showMessage(QObject::tr("No scan results to save."), 4000);
        return;
    }
    const QString path = QFileDialog::getSaveFileName(this, QObject::tr("Save current scan results"),
        "scanresults.txt", QObject::tr("Text (*.txt);;CSV (*.csv);;All files (*)"));
    if (path.isEmpty()) return;
    QFile f(path);
    if (!f.open(QIODevice::WriteOnly | QIODevice::Text)) {
        statusBar()->showMessage(QObject::tr("Could not open the file for writing."), 4000);
        return;
    }
    ...
    QTextStream out(&f);
    out << QObject::tr("Address") << sep << QObject::tr("Value") << '\n';
    ...
    QString msg = QObject::tr("Saved %1 results to %2").arg(rows).arg(QFileInfo(path).fileName());
    if (lastResult_ && lastResult_->count() > static_cast<size_t>(rows))
        msg += QObject::tr(" (list capped at %1 of %2 found)").arg(rows).arg(lastResult_->count());
    statusBar()->showMessage(msg, 6000);
}

void MainWindow::onSaveTable() {
    auto path = QFileDialog::getSaveFileName(this, QObject::tr("Save Cheat Table"), "",
        QObject::tr("Cheat Tables (*.ct);;JSON Tables (*.json);;All Files (*)"));
    if (path.isEmpty()) return;
  ...
  try {
    ...
    // std::terminate.
    QMessageBox::critical(this, QObject::tr("Save Cheat Table"),
        QObject::tr("Failed to save table:\n%1").arg(ex.what()));
  } catch (...) {
    QMessageBox::critical(this, QObject::tr("Save Cheat Table"), QObject::tr("Failed to save table (unknown error)."));
  }
}

void MainWindow::onLoadTable() {
    auto path = QFileDialog::getOpenFileName(this, QObject::tr("Load Cheat Table"), "",
        QObject::tr("Cheat Tables (*.ct);;JSON Tables (*.json);;All Files (*)"));
    if (path.isEmpty()) return;
...
void MainWindow::loadCheatTableModel(const ce::CheatTable& table) {
    ...
    if (!table.luaScript.empty()) {
        ...
        auto answer = QMessageBox::question(this, QObject::tr("Execute this lua script?"),
            QObject::tr("This cheat table contains a Lua script.\n\n"
        ...
        if (answer == QMessageBox::Yes) {
            ...
            if (!res.has_value())
                statusBar()->showMessage(
                    QObject::tr("Table Lua error: %1").arg(QString::fromStdString(res.error())), 8000);
        }
    }
...
void MainWindow::rebuildRecentMenu() {
    ...
    if (recent.isEmpty()) {
        recentMenu_->addAction(QObject::tr("(no recent tables)"))->setEnabled(false);
        return;
    }
    for (const QString& p : recent) {
        ...
        a->setToolTip(exists ? p : p + QObject::tr("  (missing)"));
        a->setEnabled(exists);
        if (exists) connect(a, &QAction::triggered, this, [this, p]() { loadTableFromPath(p); });
    }
    recentMenu_->addSeparator();
    connect(recentMenu_->addAction(QObject::tr("Clear list")), &QAction::triggered, this, [this]() {
...
void MainWindow::loadTableFromPath(const QString& path) {
    ...
    setWindowTitle(QObject::tr("Cheat Engine - %1").arg(QFileInfo(path).fileName()));
  try {
    ...
    if (fmt == ce::TableFormat::Protected) {
        bool ok = false;
        QString pw = QInputDialog::getText(this, QObject::tr("Protected Cheat Table"),
            QObject::tr("This table is password-protected (.CETRAINER).\nEnter its password:"),
            QLineEdit::Password, QString(), &ok);
        if (!ok) return;
        CheatTable table;
        if (!table.loadProtected(path.toStdString(), pw.toStdString())) {
            QMessageBox::warning(this, QObject::tr("Load Cheat Table"),
                QObject::tr("Could not decrypt the table (wrong password or corrupt file)."));
            return;
        }
    ...
    // into std::terminate.
    QMessageBox::critical(this, QObject::tr("Load Cheat Table"),
        QObject::tr("Failed to load table:\n%1").arg(ex.what()));
  } catch (...) {
    QMessageBox::critical(this, QObject::tr("Load Cheat Table"), QObject::tr("Failed to load table (unknown error)."));
  }
}
...
void MainWindow::loadAddressEntries(const QJsonArray& entries) {
    ...
    if (skippedForMissingProcess) {
        QMessageBox::warning(this, QObject::tr("Process required"),
            QObject::tr("Some active auto-assembler records were loaded inactive because no process is open."));
    }
    if (!failures.isEmpty()) {
        QMessageBox::warning(this, QObject::tr("Auto-assembler activation failed"),
            failures.join('\n'));
    }
}
...
void MainWindow::showInstructionAccesses(uintptr_t instructionAddr) {
    ...
    statusBar()->showMessage(QObject::tr("Monitoring the instruction at 0x%1…").arg(instructionAddr, 0, 16));
    QApplication::setOverrideCursor(Qt::WaitCursor);
    ...
    if (results.empty()) {
        ...
        QMessageBox::information(this, QObject::tr("What this instruction accesses"),
            QObject::tr("No data accesses were recorded for the instruction at 0x%1.\n\nIt may not have "
    ...
    auto* dlg = new QDialog(this);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->setWindowTitle(QObject::tr("Addresses accessed by 0x%1").arg(instructionAddr, 0, 16));
    dlg->resize(440, 360);
    auto* lay = new QVBoxLayout(dlg);
    lay->addWidget(new QLabel(QObject::tr("%1 distinct address(es) accessed (double-click to add to the list):")
        .arg(results.size())));
    auto* table = new QTableWidget((int)results.size(), 2, dlg);
    table->setHorizontalHeaderLabels({QObject::tr("Address"), QObject::tr("Hits")});
    ...
    connect(table, &QTableWidget::cellDoubleClicked, this, [this, table](int row, int) {
        if (auto* it = table->item(row, 0))
            addressListModel_->addEntry((uintptr_t)it->data(Qt::UserRole).toULongLong(),
                                        ce::ValueType::Int32, QObject::tr("Accessed address"));
    });
    lay->addWidget(table);
    ...
    dlg->show();
    statusBar()->showMessage(QObject::tr("%1 address(es) accessed by 0x%2")
...
int MainWindow::runAutorunScripts() {
    ...
    for (const QString& f : files) {
        ...
        if (!err.empty()) {
            ++failed;
            statusBar()->showMessage(QObject::tr("autorun %1: %2").arg(f, QString::fromStdString(err)), 8000);
        }
    }
    if (ran > 0 && failed == 0)
        statusBar()->showMessage(QObject::tr("Ran %1 autorun script(s).").arg(ran), 4000);
    return ran;
}
...
void MainWindow::startCodeFinderForAddress(uintptr_t addr, bool writesOnly, int watchSize) {
    ...
    if (wine && !codeFinderNoPrompt_ && !software) {
        auto r = QMessageBox::information(this, QObject::tr("Wine / Proton game"),
            QObject::tr("This is a Wine/Proton (Windows) game. Cheat Engine will watch the game's "
    ...
    if (!finder->start(*process_, *debugger, addr, writesOnly, watchSize, software, singleThread)) {
        QMessageBox::warning(this, QObject::tr("Code finder unavailable"),
            software ? QObject::tr("Could not start software watchpoint monitoring for this address.")
                     : QObject::tr("Could not start hardware watchpoint monitoring for this address."));
        return;
    }
    ...
    auto title = writesOnly ? QObject::tr("Find what writes") : QObject::tr("Find what accesses");
    ...
    window->setAddToList([this](uintptr_t a, const QString& desc) {
        showAdvancedOptions();
        advancedOptions_->addCode(a, desc.isEmpty() ? QObject::tr("code 0x%1").arg(a, 0, 16) : desc);
    });
...
void MainWindow::onMemoryView() {
    if (!process_) {
        statusBar()->showMessage(QObject::tr("No process attached; open one first (Ctrl+O) to view memory."), 4000);
        return;
    }
    openMemoryView(0);
}
...
void MainWindow::showDebugger() {
    if (!process_) { QMessageBox::warning(this, QObject::tr("No process"), QObject::tr("Open a process first.")); return; }
...
void MainWindow::openMonoDissector() {
    ...
    connect(w, &ce::gui::MonoDissectorWindow::addFieldRequested, this,
        ...
        bool ok = false;
        QString base = QInputDialog::getText(this, QObject::tr("Add Mono field"),
            QObject::tr("Object base address for '%1' (field at +0x%2):")
...
QWidget* MainWindow::openPanelByName(const QString& name) {
    // Dev/screenshot hook (--panel <name>): matches the menu ACTION TEXT, which is
    // translated when a UI translation is active, so the caller must pass the text
    // in the language the UI is currently showing (e.g. "Pointer scan" on an
    // English locale, "指针扫描" with the Chinese translation loaded).
...
QDialog* MainWindow::openSettingsDialog(const QString& page) {
    ...
    if (!page.isEmpty()) {
        // --settings-page <name>: also matches translated text (see openPanelByName).
...
MemoryBrowser* MainWindow::openMemoryView(uintptr_t addr) {
    ...
    browser->setBreakpointSetter([this](uintptr_t addr, bool hardware) {
        ...
        bp.size = 1;
        bp.enabled = true;
        bp.description = QObject::tr("BP @ 0x%1").arg(addr, 0, 16).toStdString();
        bpManager_.add(bp);
    });
    ...
    browser->setAddToList([this](uintptr_t addr, ce::ValueType type) {
        ...
        statusBar()->showMessage(QObject::tr("Added 0x%1 to the cheat table").arg(addr, 0, 16), 3000);
    });
...
void MainWindow::populateBrowserMenus(MemoryBrowser* b) {
    ...
    view->addAction(QObject::tr("Memory Regions"), this, [this, openAt]() {
        if (!process_) return;
    ...
    view->addAction(QObject::tr("Heap list"), this, [this, openAt]() {
        if (!process_) return;
    ...
    view->addAction(QObject::tr("Module list"), this, [this, openAt]() {
        if (!process_) return;
    ...
    view->addAction(QObject::tr("Referenced strings / functions"), this, [this, openAt]() {
        if (!process_) return;
    ...
    view->addAction(QObject::tr("Thread list"), this, [this]() {
        if (!process_) return;
        auto* w = new ThreadListWindow(process_.get(), this);
        w->setAttribute(Qt::WA_DeleteOnClose); w->show();
    });
    view->addAction(QObject::tr("Stacktrace"), this, [this]() {
        if (!process_) return;
        auto* w = new StackViewWindow(process_.get(), this);
        w->setAttribute(Qt::WA_DeleteOnClose); w->show();
    });
    view->addAction(QObject::tr("Graphical memory view"), this, [this]() {
        if (!process_) return;
    ...
    if (dbg) {
        dbg->addSeparator();
        dbg->addAction(QObject::tr("Registers"), this, [this]() {
            if (!process_) return;
            auto* w = new RegisterEditorWindow(process_.get(), this);
            w->setAttribute(Qt::WA_DeleteOnClose); w->show();
        });
        dbg->addAction(QObject::tr("Breakpoint list"), this, [this]() {
            auto* w = new BreakpointListWindow(&bpManager_, this);
            w->setAttribute(Qt::WA_DeleteOnClose); w->show();
        });
        dbg->addAction(QObject::tr("Full debugger"), this, [this]() { showDebugger(); });
        dbg->addAction(QObject::tr("Break and trace"), this, [this]() {
            if (!process_) return;
        ...
        dbg->addAction(QObject::tr("Branch mapper (LBR)"), this, [this]() {
            if (!process_) return;
...
void MainWindow::addAnalysisToolsMenu(QMenu* tools) {
    tools->addAction(QObject::tr("Auto Assemble"), this, [this]() {
        auto* editor = new ScriptEditor(process_.get(), &autoAsm_, this);
        editor->setAttribute(Qt::WA_DeleteOnClose);
    ...
    tools->addAction(QObject::tr("Pointer scan"), this, [this]() {
        if (!process_) return;
        auto* dlg = new PointerScanDialog(process_.get(), this);
        connect(dlg, &PointerScanDialog::addressSelected, this,
    ...
    tools->addAction(QObject::tr("Emulator guest scan"), this, [this]() {
        if (!process_) { QMessageBox::warning(this, QObject::tr("No process"), QObject::tr("Open a process first.")); return; }
    ...
    tools->addAction(QObject::tr("Dissect data/structures"), this, [this]() {
        if (!process_) return;
    ...
    tools->addAction(QObject::tr("Find static addresses"), this, [this]() {
        if (!process_) { QMessageBox::warning(this, QObject::tr("No process"), QObject::tr("Open a process first.")); return; }
        auto* w = new FindStaticsWindow(process_.get(), this);
        w->setAttribute(Qt::WA_DeleteOnClose); w->show();
    });
    tools->addAction(QObject::tr("Mono dissector..."), this, &MainWindow::openMonoDissector);
    tools->addAction(QObject::tr("Lua Engine"), this, [this]() {
        luaEngine_.setProcess(process_.get());
    ...
    tools->addAction(QObject::tr("Find Statics / ELF Inspector"), this, [this]() {
        QString initial;
...
void MainWindow::updateScanButtons() {
    ...
    firstScanBtn_->setText(scanActive ? QObject::tr("New Scan") : QObject::tr("First Scan"));
...
QVariant ScanResultsModel::headerData(int section, Qt::Orientation o, int role) const {
    ...
    switch (section) {
        case 0:  return QObject::tr("Address");
        case 1:  return QObject::tr("Value");       // current (live) value
        default: return QObject::tr("Previous");    // value captured at scan time
    }
}
...
void AddressListModel::addScriptEntry(const QString& desc, const QString& script) {
    ...
    AddressEntry entry;
    entry.id = allocId();
    entry.description = desc.isEmpty() ? QObject::tr("Auto Assembler script") : desc;
    entry.autoAsmScript = script;   // toggling the checkbox runs [ENABLE]/[DISABLE]
    entry.type = ValueType::Int32;
    entry.currentValue = QObject::tr("(Auto Assembler script)");
    entries_.push_back(std::move(entry));
    endInsertRows();
}
...
bool AddressListModel::setEntryActive(int row, bool active) {
    ...
    if (!e.autoAsmScript.isEmpty()) {
        if (!proc_ || !autoAsm_) {
            reportActivationError(QObject::tr("Process required"),
        ...
        if (active) {
            ...
            if (!result.success) {
                reportActivationError(QObject::tr("Auto-assembler activation failed"),
            ...
            if (!result.success) {
                reportActivationError(QObject::tr("Auto-assembler deactivation failed"),
                    QString::fromStdString(result.error));
                return false;
            }
...
QVariant AddressListModel::headerData(int section, Qt::Orientation o, int role) const {
    if (role != Qt::DisplayRole || o != Qt::Horizontal) return {};
    switch (section) {
        case 0: return QObject::tr("Active");
        case 1: return QObject::tr("Description");
        case 2: return QObject::tr("Address");
        case 3: return QObject::tr("Type");
        case 4: return QObject::tr("Value");
        default: return {};
    }
}
