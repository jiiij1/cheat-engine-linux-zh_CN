...
void HexView::contextMenuEvent(QContextMenuEvent* e) {
    ...
    QMenu menu(this);
    menu.addAction(QObject::tr("Address: 0x%1").arg(addr, 16, 16, QChar('0')))->setEnabled(false);
    menu.addSeparator();

    auto* copyAddr = menu.addAction(QObject::tr("Copy address"));
    auto* copyByte = haveByte
        ? menu.addAction(QObject::tr("Copy byte (0x%1)").arg(selByte, 2, 16, QChar('0')))
        : nullptr;
    ...
    QAction* copySelAob = selCount > 1
        ? menu.addAction(QObject::tr("Copy selection as AOB (%1 bytes)").arg(selCount)) : nullptr;
    QAction* copySelHex = selCount > 1
        ? menu.addAction(QObject::tr("Copy selection (hex, no spaces)")) : nullptr;
    QAction* fillSel = selCount > 1
        ? menu.addAction(QObject::tr("Fill selection (%1 bytes) with…").arg(selCount)) : nullptr;
    auto* copyAob = menu.addAction(QObject::tr("Copy 16 bytes as AOB"));
    auto* gotoAct = menu.addAction(QObject::tr("Goto…"));
    auto* followPtr = menu.addAction(QObject::tr("Follow pointer here (qword)"));
    auto* addToList = menu.addAction(QObject::tr("Add address to the list"));
    ...
    QAction* pasteAct = clipAob.empty() ? nullptr
        : menu.addAction(QObject::tr("Paste %1 bytes here").arg(clipAob.size()));

    menu.addSeparator();
    auto* findWrites = menu.addAction(QObject::tr("Find what writes this address"));
    auto* findAccesses = menu.addAction(QObject::tr("Find what accesses this address"));
    ...
    menu.addSeparator();
    auto* bprMenu = menu.addMenu(QObject::tr("Bytes per row"));
    ...
    auto* dtMenu = menu.addMenu(QObject::tr("Display type"));
    const std::pair<QString, DisplayType> dtChoices[] = {
        {QObject::tr("Byte"), DisplayType::Byte}, {QObject::tr("Word (2)"), DisplayType::Word},
        {QObject::tr("Dword (4)"), DisplayType::Dword}, {QObject::tr("Qword (8)"), DisplayType::Qword},
        {QObject::tr("Float"), DisplayType::Float}, {QObject::tr("Double"), DisplayType::Double},
    };
    ...
    if (picked == copyAddr) {
        ...
        bool ok = false;
        QString v = QInputDialog::getText(this, QObject::tr("Fill selection"), QObject::tr("Fill byte (hex):"),
...
void DisasmView::contextMenuEvent(QContextMenuEvent* e) {
    ...
    QMenu menu(this);
    menu.addAction(QObject::tr("Address: 0x%1").arg(inst.address, 16, 16, QChar('0')))->setEnabled(false);
    menu.addSeparator();

    auto* copyAddr = menu.addAction(QObject::tr("Copy address"));
    auto* copyBytes = menu.addAction(multi ? QObject::tr("Copy bytes (%1 instructions)").arg(selInsts.size())
                                           : QObject::tr("Copy bytes"));
    auto* copyLine = menu.addAction(multi ? QObject::tr("Copy lines (%1)").arg(selInsts.size())
                                          : QObject::tr("Copy line"));
    menu.addSeparator();
    ...
    if ((isBranch && parseImmediate(inst.operands)) || ripEffectiveAddress(inst))
        followAct = menu.addAction(QObject::tr("Follow operand"));
    ...
    if (haveBp) {
        removeBp = menu.addAction(QObject::tr("Remove breakpoint"));
    } else {
        setSwBp = menu.addAction(QObject::tr("Set software breakpoint"));
        setHwBp = menu.addAction(QObject::tr("Set hardware exec breakpoint"));
    }

    menu.addSeparator();
    auto* labelAct = menu.addAction(QObject::tr("Label this address…"));
    auto* commentAct = menu.addAction(QObject::tr("Set comment…"));
    auto* addListAct = menu.addAction(QObject::tr("Add this address to the list"));
    auto* xrefAct = menu.addAction(QObject::tr("Find references to this address"));
    auto* accessAct = menu.addAction(QObject::tr("Find out what addresses this instruction accesses"));
    auto* asmAct = menu.addAction(QObject::tr("Assemble instruction…"));
    auto* nopAct = menu.addAction(multi
        ? QObject::tr("NOP %1 instructions (%2 bytes)").arg(selInsts.size()).arg(selTotalBytes)
        : QObject::tr("NOP this instruction (%1 byte%2)").arg(inst.size).arg(inst.size == 1 ? "" : "s"));
    // Auto Assemble injection templates, pre-filled for the pointed-at instruction.
    auto* aaMenu = menu.addMenu(QObject::tr("Auto Assemble"));
    auto* codeInjAct = aaMenu->addAction(QObject::tr("Create code injection here"));
    auto* aobInjAct = aaMenu->addAction(QObject::tr("Create AOB injection here"));
    auto* saveAct = menu.addAction(QObject::tr("Save region to file…"));
    auto* loadAct = menu.addAction(QObject::tr("Load region from file…"));
...
void DisasmView::refresh() {
    ...
    if (!r || *r == 0) {
        ...
        emptyReason_ = denied
            ? QObject::tr("Cannot read this process's memory (permission denied).\n"
                             "Run Cheat Engine with ptrace rights: sudo setcap "
                             "cap_sys_ptrace+ep <cheatengine>, run as root, or set "
                             "kernel.yama.ptrace_scope=0.")
            : QObject::tr("No readable memory at this address.");
        viewport()->update();
        return;
    }
...
void DisasmView::paintEvent(QPaintEvent*) {
    ...
    if (instructions_.empty()) {
        ...
        p.drawText(viewport()->rect().adjusted(24, 24, -24, -24),
                   ...
                   emptyReason_.isEmpty() ? QObject::tr("No data at this address.")
                                          : emptyReason_);
        return;
    }
        ...
        if (inst.address == currentIp_ && currentCtx_.rflags) {
            ...
            if (inst.mnemonic != "jmp") {
                if (auto jt = ce::conditionalJumpTaken(inst.mnemonic, currentCtx_.rflags))
                    annotation += *jt ? QObject::tr("  (will jump)") : QObject::tr("  (no jump)");
            }
        }
...
void MemoryBrowser::buildMenuBar() {
    ...
    auto* file = mb->addMenu(QObject::tr("&File"));
    file->addAction(QObject::tr("New window"), this, [this]() {
    ...
    file->addAction(QObject::tr("Save region to file…"), this, [this]() { saveRegionToFile(currentAddr_); });
    file->addAction(QObject::tr("Load region from file…"), this, [this]() { loadRegionFromFile(currentAddr_); });

    auto* search = mb->addMenu(QObject::tr("&Search"));
    { auto* a = search->addAction(QObject::tr("Find memory..."), this, [this]() { findInMemory(false); });
      a->setShortcut(QKeySequence::Find); }
    { auto* a = search->addAction(QObject::tr("Find next"), this, [this]() { findInMemory(true); });
      a->setShortcut(QKeySequence("F3")); }
    { auto* a = search->addAction(QObject::tr("Find previous"), this, [this]() { findInMemory(true, /*backward=*/true); });
      a->setShortcut(QKeySequence("Shift+F3")); }

    viewMenu_ = mb->addMenu(QObject::tr("&View"));
    viewMenu_->addAction(QObject::tr("Disassembler preferences..."), this, [this]() {
    ...
    debugMenu_ = mb->addMenu(QObject::tr("&Debug"));
    { auto* a = debugMenu_->addAction(QObject::tr("Toggle breakpoint"), this, [this]() {
    ...
    { auto* a = debugMenu_->addAction(QObject::tr("Run"), this, [this]() { if (runFn_) runFn_(); });
      a->setShortcut(QKeySequence("F9")); }
    { auto* a = debugMenu_->addAction(QObject::tr("Step into"), this, [this]() { if (stepIntoFn_) stepIntoFn_(); });
      a->setShortcut(QKeySequence("F7")); }
    { auto* a = debugMenu_->addAction(QObject::tr("Step over"), this, [this]() { if (stepOverFn_) stepOverFn_(); });
      a->setShortcut(QKeySequence("F8")); }
    { auto* a = debugMenu_->addAction(QObject::tr("Run to cursor"), this, [this]() {
          uintptr_t addr = disasmView_->selectedAddress();
          if (!addr) addr = currentAddr_;
    ...
    toolsMenu_ = mb->addMenu(QObject::tr("&Tools"));
    toolsMenu_->addAction(QObject::tr("Auto Assemble..."), this, [this]() {
        if (autoAssembleOpener_) autoAssembleOpener_(QString());   // empty script editor
    });
    toolsMenu_->addAction(QObject::tr("Dissect data/structures..."), this, [this]() {
...
MemoryBrowser::MemoryBrowser(ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Memory Viewer"));
    resize(900, 600);
    buildMenuBar();   // CE keeps the memory/debug tools in this window's menu bar

    // Toolbar
    auto* toolbar = new QToolBar;
    backAct_ = toolbar->addAction(QObject::tr("◀"));
    backAct_->setToolTip(QObject::tr("Back (Alt+Left)"));
    backAct_->setEnabled(false);
    connect(backAct_, &QAction::triggered, this, &MemoryBrowser::goBack);
    fwdAct_ = toolbar->addAction(QObject::tr("▶"));
    fwdAct_->setToolTip(QObject::tr("Forward (Alt+Right)"));
    fwdAct_->setEnabled(false);
    connect(fwdAct_, &QAction::triggered, this, &MemoryBrowser::goForward);
    toolbar->addSeparator();
    toolbar->addWidget(new QLabel(QObject::tr(" Address: ")));
    ...
    auto* goBtn = new QPushButton(QObject::tr("Go"));
    connect(goBtn, &QPushButton::clicked, this, &MemoryBrowser::onGotoAddress);
    toolbar->addWidget(goBtn);

    toolbar->addSeparator();
    auto* refreshBtn = new QPushButton(QObject::tr("Refresh"));
    connect(refreshBtn, &QPushButton::clicked, this, &MemoryBrowser::onRefresh);
    toolbar->addWidget(refreshBtn);

    toolbar->addSeparator();
    auto* bmToggleBtn = new QPushButton(QObject::tr("★"));
    bmToggleBtn->setToolTip(QObject::tr("Toggle bookmark at current address (Ctrl+B)"));
    ...
    bmBtn->setText(QObject::tr("Bookmarks ▾"));
    ...
    auto* bpAct = dbgBar->addAction(QObject::tr("Toggle BP"));
    bpAct->setToolTip(QObject::tr("Toggle breakpoint at the selected instruction (F5)"));
    ...
    { auto* a = dbgBar->addAction(QObject::tr("Run"));
      a->setToolTip(QObject::tr("Continue the paused target (F9)"));
      connect(a, &QAction::triggered, this, [this]() { if (runFn_) runFn_(); }); }
    { auto* a = dbgBar->addAction(QObject::tr("Step"));
      a->setToolTip(QObject::tr("Step into one instruction (F7)"));
      connect(a, &QAction::triggered, this, [this]() { if (stepIntoFn_) stepIntoFn_(); }); }
    { auto* a = dbgBar->addAction(QObject::tr("Step over"));
      a->setToolTip(QObject::tr("Step over one instruction (F8)"));
      connect(a, &QAction::triggered, this, [this]() { if (stepOverFn_) stepOverFn_(); }); }
    { auto* a = dbgBar->addAction(QObject::tr("Run to cursor"));
      a->setToolTip(QObject::tr("Continue until the selected instruction (F4)"));
    ...
    auto* dbgAct = dbgBar->addAction(QObject::tr("Debugger"));
    dbgAct->setToolTip(QObject::tr("Open the Debugger here to run, step, and set breakpoints"));
    ...
    auto* prefsAct = dbgBar->addAction(QObject::tr("Preferences"));
    prefsAct->setToolTip(QObject::tr("Disassembler Preferences: font and colors"));
    ...
    registerPanel_->setToolTip(QObject::tr("Registers (populated at a debug breakpoint)"));
    ...
    stacktracePanel_->setHorizontalHeaderLabels(
        {QObject::tr("Address"), is32 ? "DWORD" : "QWORD", QObject::tr("Value")});
    ...
    stacktracePanel_->setToolTip(QObject::tr("Call stack (populated at a debug breakpoint)"));
    ...
    if (viewMenu_ && registerPanel_ && stacktracePanel_) {
        ...
        auto* cpuAct = viewMenu_->addAction(QObject::tr("CPU registers && stack panels"));
    ...
    connect(hexView_, &HexView::requestFindWhatAccesses, this, [this](uintptr_t addr, bool writesOnly) {
        ...
        else QMessageBox::information(this, QObject::tr("Code finder unavailable"),
            QObject::tr("No code-finder launcher is wired to this memory browser instance."));
    });
    ...
    connect(hexView_, &HexView::cursorMoved, this, [this](uintptr_t addr) {
        ...
        if (got < 1) { statusBar()->showMessage(head + QObject::tr("<unreadable>")); return; }
        QString msg = head;
        ...
        if (got >= 4) {
            ...
            msg += QObject::tr("  i32=%1  float=%2").arg(v).arg((double)f, 0, 'g', 7);
        }
        if (got >= 8) {
            int64_t v; std::memcpy(&v, b, 8);
            double d; std::memcpy(&d, b, 8);
            msg += QObject::tr("  i64=%1  double=%2").arg(v).arg(d, 0, 'g', 10);
        }
    ...
    connect(disasmView_, &DisasmView::requestSetComment, this, [this](uintptr_t addr) {
        ...
        QString text = QInputDialog::getText(this, QObject::tr("Comment"),
            QObject::tr("Comment for 0x%1 (blank to clear):").arg(addr, 0, 16),
    ...
    connect(disasmView_, &DisasmView::requestSetSymbol, this, [this](uintptr_t addr) {
        ...
        bool ok = false;
        QString name = QInputDialog::getText(this, QObject::tr("Label address"),
            QObject::tr("Name for 0x%1 (blank to clear):").arg(addr, 0, 16),
    ...
    connect(disasmView_, &DisasmView::requestInjection, this, [this](uintptr_t addr, bool aob) {
        ...
        if (script.empty()) {
            QMessageBox::warning(this, QObject::tr("Auto Assemble"),
    ...
    connect(gotoShortcut, &QShortcut::activated, this, [this]() {
        bool ok;
        auto text = QInputDialog::getText(this, QObject::tr("Goto Address"),
            QObject::tr("Address, symbol, or expression (e.g. worker, target2+0x100):"),
    ...
    if (proc_) {
        ...
        if (!pr || *pr != 1)
            statusBar()->showMessage(
                QObject::tr("0x%1 is not readable (unmapped or protected page)").arg(addr, 0, 16), 5000);
    }
}
...
void MemoryBrowser::detachFromTarget() {
    ...
    setWindowTitle(QObject::tr("Memory Viewer (target exited)"));
    statusBar()->showMessage(QObject::tr("Target process exited; this view is frozen."), 0);
}
...
void MemoryBrowser::rebuildBookmarksMenu() {
    ...
    if (bookmarks_.empty()) {
        bookmarksMenu_->addAction(QObject::tr("(no bookmarks; Ctrl+B to add)"))->setEnabled(false);
        return;
    }
    ...
    bookmarksMenu_->addAction(QObject::tr("Clear all bookmarks"), this, [this]() {
        bookmarks_.clear();
        rebuildBookmarksMenu();
    });
}
...
void MemoryBrowser::onGotoAddress() {
    ...
    else QMessageBox::warning(this, QObject::tr("Goto"), QObject::tr("Could not resolve \"%1\".").arg(text));
}
...
void MemoryBrowser::writeNop(uintptr_t addr, int size) {
    ...
    if (!patchBytes(addr, nops)) {
        QMessageBox::warning(this, QObject::tr("NOP failed"),
            QObject::tr("Could not write %1 NOP byte%2 to 0x%3.")
...
void MemoryBrowser::assembleAt(uintptr_t addr, int origSize, const QString& current) {
    if (!proc_) return;
    bool ok = false;
    QString text = QInputDialog::getText(this, QObject::tr("Assemble"),
        QObject::tr("Instruction to assemble at 0x%1:").arg(addr, 0, 16),
    ...
    if (!res) {
        QMessageBox::warning(this, QObject::tr("Assemble failed"),
            QObject::tr("Could not assemble \"%1\":\n%2").arg(text, QString::fromStdString(res.error())));
        return;
    }
    std::vector<uint8_t> bytes = *res;
    if (bytes.empty()) {
        QMessageBox::warning(this, QObject::tr("Assemble failed"),
            QObject::tr("\"%1\" assembled to no bytes; not writing.").arg(text));
        return;
    }
    if ((int)bytes.size() > origSize) {
        auto ans = QMessageBox::question(this, QObject::tr("Longer instruction"),
            QObject::tr("The new instruction is %1 bytes but the original was %2. Writing it will "
    ...
    if (!patchBytes(addr, bytes)) {
        QMessageBox::warning(this, QObject::tr("Write failed"),
            QObject::tr("Assembled %1 bytes but could not write to 0x%2.").arg(bytes.size()).arg(addr, 0, 16));
        return;
    }
    onRefresh();
}
...
void MemoryBrowser::findInMemory(bool findNext, bool backward) {
    ...
    if (findNext && !lastSearch_.empty()) {
        ...
        bool ok = false;
        QString s = QInputDialog::getText(this, QObject::tr("Find in memory"),
            QObject::tr("Bytes (e.g. 48 8B ?? 05) or \"text\":"), QLineEdit::Normal, QObject::tr(""), &ok);
        ...
        if (s.startsWith('"') && s.endsWith('"') && s.size() >= 2) {
            ...
            if (bytes.empty()) {
                QMessageBox::warning(this, QObject::tr("Find"),
                    QObject::tr("Enter hex bytes (48 8B 05), a wildcard AOB (48 8B ?? 05), or a quoted \"string\"."));
                return;
            }
            for (int b : bytes) {
    ...
    if (hit) {
        ...
        QMessageBox::information(this, QObject::tr("Find"),
            backward ? QObject::tr("Not found searching backward from the current address.")
                     : QObject::tr("Not found searching forward from the current address."));
    }
}
...
void MemoryBrowser::showXrefs(uintptr_t addr) {
    ...
    if (!found) {
        QMessageBox::information(this, QObject::tr("References"),
            QObject::tr("This address is not inside a known module, so its references can't be scanned."));
        return;
    }
    ...
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->setWindowTitle(QObject::tr("References to 0x%1: %2 found").arg(addr, 0, 16).arg(refs.size()));
    dlg->resize(640, 360);
    auto* lay = new QVBoxLayout(dlg);
    auto* table = new QTableWidget(dlg);
    table->setColumnCount(3);
    table->setHorizontalHeaderLabels({QObject::tr("From"), QObject::tr("Type"), QObject::tr("Instruction")});
...
void MemoryBrowser::saveRegionToFile(uintptr_t addr) {
    if (!proc_) return;
    bool ok = false;
    int size = QInputDialog::getInt(this, QObject::tr("Save region"), QObject::tr("Bytes to save:"), 4096, 1, 1 << 28, 1, &ok);
    if (!ok) return;
    auto path = QFileDialog::getSaveFileName(this, QObject::tr("Save region"),
        QObject::tr("region_%1_%2.bin").arg(addr, 0, 16).arg(size), QObject::tr("Binary (*.bin);;All (*)"));
    if (path.isEmpty()) return;

    std::vector<uint8_t> buf(size);
    auto r = proc_->read(addr, buf.data(), size);
    if (!r) {
        QMessageBox::warning(this, QObject::tr("Read failed"),
            QObject::tr("Could not read %1 bytes at 0x%2.").arg(size).arg(addr, 0, 16));
        return;
    }
    QFile f(path);
...
void MemoryBrowser::loadRegionFromFile(uintptr_t addr) {
    if (!proc_) return;
    auto path = QFileDialog::getOpenFileName(this, QObject::tr("Load region into memory"), QString(),
                                             QObject::tr("Binary (*.bin);;All (*)"));
    if (path.isEmpty()) return;
    QFile f(path);
    if (!f.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(this, QObject::tr("Load failed"), f.errorString());
        return;
    }
    ...
    if (f.size() > kMaxLoad) {
        QMessageBox::warning(this, QObject::tr("File too large"),
            QObject::tr("The file is %1 bytes; the maximum region load is %2 bytes.")
    ...
    if (QMessageBox::question(this, QObject::tr("Load region"),
            QObject::tr("Write %1 bytes from\n%2\nto 0x%3?")
    ...
    if (!w || *w < static_cast<size_t>(data.size())) {
        QMessageBox::warning(this, QObject::tr("Write failed"),
            QObject::tr("Wrote %1 of %2 bytes at 0x%3 (memory may be protected).")
