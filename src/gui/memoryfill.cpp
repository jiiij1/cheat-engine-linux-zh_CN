...
MemoryFillDialog::MemoryFillDialog(ProcessHandle* proc, uintptr_t startAddr, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Fill Memory"));
    resize(350, 150);
    ...
    form->addRow(QObject::tr("Start address:"), addrEdit_);

    sizeEdit_ = new QLineEdit("256");
    form->addRow(QObject::tr("Size (bytes):"), sizeEdit_);

    valueEdit_ = new QLineEdit("00");
    valueEdit_->setFont(QFont("Monospace", 10));
    form->addRow(QObject::tr("Fill byte (hex):"), valueEdit_);
    layout->addLayout(form);

    auto* btnRow = new QHBoxLayout;
    auto* fillBtn = new QPushButton(QObject::tr("Fill"));
...
void MemoryFillDialog::onFill() {
    ...
    if (size > kMaxFillSize) {
        QMessageBox::warning(this, QObject::tr("Fill Memory"),
            QObject::tr("Fill size too large (max %1 bytes).").arg(kMaxFillSize));
        return;
    }
    ...
    if (r)
        QMessageBox::information(this, QObject::tr("Fill Memory"), QObject::tr("Filled %1 bytes at 0x%2 with 0x%3")
            .arg(size).arg(addr, 0, 16).arg(fillByte, 2, 16, QChar('0')));
    else
        QMessageBox::warning(this, QObject::tr("Fill Memory"), QObject::tr("Write failed"));
    accept();
}
