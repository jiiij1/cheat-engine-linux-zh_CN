...
MonoDissectorWindow::MonoDissectorWindow(ce::ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Mono Dissector"));
    resize(720, 640);

    auto* bar = new QToolBar;
    auto* dissectAct = bar->addAction(QObject::tr("Dissect (inject agent)"));
    dissectAct->setToolTip(QObject::tr("Inject the Mono agent and read the target's class/field layout"));
    connect(dissectAct, &QAction::triggered, this, &MonoDissectorWindow::runDissect);
    bar->addSeparator();
    bar->addWidget(new QLabel(QObject::tr(" Filter: ")));
    filter_ = new QLineEdit;
    filter_->setPlaceholderText(QObject::tr("class or field name…"));
    ...
    tree_->setHeaderLabels({QObject::tr("Name"), QObject::tr("Offset"), QObject::tr("Type")});
    ...
    v->addWidget(tree_);
    status_ = new QLabel(QObject::tr("Click \"Dissect\" to read the target's Mono layout."));
    v->addWidget(status_);
...
void MonoDissectorWindow::setDissection(const ce::MonoDissection& d) {
    ...
    for (const auto& img : d.images) {
        ...
        for (const auto& c : img.classes) {
            ...
            for (const auto& f : c.fields) {
                QString name = QString::fromStdString(f.name) + (f.isStatic ? QObject::tr(" (static)") : "");
...
void MonoDissectorWindow::runDissect() {
    ...
    switch (ce::detectManagedKind(*proc_)) {
        case ce::ManagedKind::Il2Cpp: {
            ...
            status_->setText(QObject::tr("IL2CPP target: locating metadata + GameAssembly…"));
            QApplication::setOverrideCursor(Qt::WaitCursor);
            QApplication::processEvents();
            auto layout = ce::resolveIl2CppForProcess(*proc_);
            QApplication::restoreOverrideCursor();
            if (!layout.ok) {
                status_->setText(QObject::tr("IL2CPP: ") + QString::fromStdString(layout.error));
                return;
            }
        ...
        case ce::ManagedKind::None:
            status_->setText(QObject::tr("No Mono runtime detected in this process."));
            return;
    ...
    if (agent.empty()) {
        status_->setText(QObject::tr("libcecore_mono_agent.so not found next to the binary."));
        return;
    }
    status_->setText(QObject::tr("Injecting agent and reading Mono layout…"));
    QApplication::setOverrideCursor(Qt::WaitCursor);
    QApplication::processEvents();
    ...
    QApplication::restoreOverrideCursor();
    if (!d) { status_->setText(QObject::tr("Agent injection failed (see CE_LOG=general:debug).")); return; }
    setDissection(*d);
}
