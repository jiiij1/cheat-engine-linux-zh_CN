...
PointerScanDialog::PointerScanDialog(ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Pointer Scanner"));
    resize(700, 500);
    ...
    configRow->addWidget(new QLabel(QObject::tr("Target Address:")));
    ...
    configRow->addWidget(new QLabel(QObject::tr("Depth:")));
    depthSpin_ = new QSpinBox;
    depthSpin_->setRange(1, 7);
    depthSpin_->setValue(4);
    configRow->addWidget(depthSpin_);

    configRow->addWidget(new QLabel(QObject::tr("Max Offset:")));
    ...
    scanBtn_ = new QPushButton(QObject::tr("Scan"));
    ...
    rescanBtn_ = new QPushButton(QObject::tr("Rescan…"));
    rescanBtn_->setToolTip(QObject::tr("Filter current paths to those that still point at a new target address"));
...
void PointerScanDialog::onScan() {
    ...
    if (!ok) { statusLabel_->setText(QObject::tr("Invalid address")); return; }

    scanBtn_->setEnabled(false);
    statusLabel_->setText(QObject::tr("Scanning..."));
    ...
    if (results_.size() > 1000)
        statusLabel_->setText(QObject::tr("Found %1 paths (showing first 1000)").arg(results_.size()));
    else
        statusLabel_->setText(QObject::tr("Found %1 paths").arg(results_.size()));
...
void PointerScanDialog::populateResults() {
    ...
    for (size_t i = 0; i < shown; ++i) {
        ...
        resultsTable_->setItem(i, 1, new QTableWidgetItem(
            addr ? QString("0x%1").arg(addr, 0, 16) : QObject::tr("??")));
        if (addr) {
            int32_t val = 0;
...
void PointerScanDialog::onRescan() {
    ...
    bool ok = false;
    QString text = QInputDialog::getText(this, QObject::tr("Rescan pointer paths"),
        QObject::tr("New target address (hex) the paths should now resolve to:"),
        QLineEdit::Normal, targetEdit_->text(), &ok);
    if (!ok) return;
    uintptr_t newTarget = text.toULongLong(&ok, 16);
    if (!ok || !newTarget) { statusLabel_->setText(QObject::tr("Invalid address")); return; }
    ...
    populateResults();
    statusLabel_->setText(QObject::tr("Rescan: %1 → %2 paths still reach 0x%3")
        .arg(before).arg(results_.size()).arg(newTarget, 0, 16));
}

void PointerScanDialog::onSave() {
    if (results_.empty()) return;
    auto path = QFileDialog::getSaveFileName(this, QObject::tr("Save pointer paths"), "pointers.ptr",
        QObject::tr("Pointer scan (*.ptr);;All (*)"));
    if (path.isEmpty()) return;
    if (!ce::savePointerPaths(path.toStdString(), results_))
        QMessageBox::warning(this, QObject::tr("Save failed"), QObject::tr("Could not write the pointer paths."));
    else
        statusLabel_->setText(QObject::tr("Saved %1 paths").arg(results_.size()));
}

void PointerScanDialog::onLoad() {
    auto path = QFileDialog::getOpenFileName(this, QObject::tr("Load pointer paths"), "",
        QObject::tr("Pointer scan (*.ptr);;All (*)"));
    if (path.isEmpty()) return;
    std::string err;
    ...
    populateResults();
    statusLabel_->setText(QObject::tr("Loaded %1 paths").arg(results_.size()));
}
