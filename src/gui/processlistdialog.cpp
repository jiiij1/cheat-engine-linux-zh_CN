...
ProcessListDialog::ProcessListDialog(QWidget* parent) : QDialog(parent) {
    setWindowTitle(QObject::tr("Open Process"));
    resize(400, 500);
    ...
    filterEdit_->setPlaceholderText(QObject::tr("Type to filter by name (e.g. warband)..."));
    ...
    tabs_->addTab(processList_, QObject::tr("Processes"));
    layout->addWidget(tabs_);

    // Buttons
    auto* btnLayout = new QHBoxLayout;
    auto* openBtn = new QPushButton(QObject::tr("Open"));
...
void ProcessListDialog::refreshList() {
    ...
    for (auto& r : rows) {
        ...
        QString mem = r.rss >= (1u << 20)
            ? QObject::tr("%1 MB").arg(r.rss / (1024.0 * 1024.0), 0, 'f', 1)
            : QObject::tr("%1 KB").arg(r.rss / 1024.0, 0, 'f', 0);
        ...
        auto text = QObject::tr("%1   (pid %2, %3)%4")
                        .arg(QString::fromStdString(p.name)).arg(p.pid).arg(mem)
                        .arg(p.sandboxed ? QObject::tr("   [sandboxed]") : QString());
        ...
        if (p.sandboxed)
            tip += (tip.isEmpty() ? QString() : QStringLiteral("\n")) +
                   QObject::tr("Sandboxed (Flatpak/Snap/container): attaching reaches into "
