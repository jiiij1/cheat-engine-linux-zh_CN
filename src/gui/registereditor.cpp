...
RegisterEditorWindow::RegisterEditorWindow(ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Register Editor"));
    resize(420, 620);
    ...
    auto* refreshButton = new QPushButton(QObject::tr("Refresh"));
    auto* applyButton = new QPushButton(QObject::tr("Apply"));
    ...
    table_->setHorizontalHeaderLabels({QObject::tr("Register"), QObject::tr("Value")});
    ...
    auto* fpLabel = new QLabel(QObject::tr("Floating point / SIMD registers"));
    layout->addWidget(fpLabel);

    fpTable_ = new QTableWidget;
    fpTable_->setColumnCount(3);
    fpTable_->setHorizontalHeaderLabels({QObject::tr("Register"), QObject::tr("XMM low 128"), QObject::tr("YMM high 128")});
    ...
    for (int row = 0; row < 16; ++row) {
        fpTable_->setItem(row, 0, new QTableWidgetItem(QObject::tr("XMM%1/YMM%1").arg(row)));
...
void RegisterEditorWindow::refreshRegisters() {
    ...
    if (!attached) {
        statusLabel_->setText(QObject::tr("Could not attach to thread."));
        return;
    }
    ...
    if (!context) {
        debugger.detach();
        statusLabel_->setText(QObject::tr("Could not read thread context."));
        return;
    }
    ...
    debugger.detach();
    statusLabel_->setText(QObject::tr("Loaded TID %1").arg(tid));
}
...
void RegisterEditorWindow::refreshFloatingPointRegisters(pid_t tid) {
    ...
    if (ptrace(PTRACE_GETREGSET, tid, (void*)NT_X86_XSTATE, &iov) < 0) {
        auto error = QObject::tr("unavailable: %1").arg(strerror(errno));
...
void RegisterEditorWindow::applyRegisters() {
    ...
    for (int row = 0; row < (int)std::size(kRegisters); ++row) {
        ...
        if (!ok) {
            statusLabel_->setText(QObject::tr("Invalid %1 value.").arg(kRegisters[row].name));
            return;
        }
    ...
    debugger.detach();
    if (!applied) {
        statusLabel_->setText(QObject::tr("Could not apply register context."));
        return;
    }

    context_ = updated;
    statusLabel_->setText(QObject::tr("Applied TID %1").arg(tid));
}
