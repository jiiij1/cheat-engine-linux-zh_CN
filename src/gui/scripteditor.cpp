...
ScriptEditor::ScriptEditor(ProcessHandle* proc, AutoAssembler* autoAsm, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Auto Assembler"));
    resize(700, 500);

    // Toolbar
    auto* toolbar = new QToolBar;
    executeBtn_ = new QPushButton(QObject::tr("Execute"));
    ...
    disableBtn_ = new QPushButton(QObject::tr("Disable"));
    disableBtn_->setEnabled(false);
    connect(disableBtn_, &QPushButton::clicked, this, &ScriptEditor::onDisable);

    auto* checkBtn = new QPushButton(QObject::tr("Syntax Check"));
    connect(checkBtn, &QPushButton::clicked, this, &ScriptEditor::onCheck);

    auto* addTableBtn = new QPushButton(QObject::tr("Add to Cheat Table"));
    addTableBtn_ = addTableBtn;
    addTableBtn->setToolTip(QObject::tr("Save this script as a cheat-table entry whose checkbox "
                            "enables/disables it."));
    ...
    connect(loadBtn, &QPushButton::clicked, this, [this]() {
        auto path = QFileDialog::getOpenFileName(this, QObject::tr("Load Script"), "", QObject::tr("CE Scripts (*.cea *.asm);;All Files (*)"));
    ...
    auto* saveBtn = new QPushButton(QObject::tr("Save"));
    connect(saveBtn, &QPushButton::clicked, this, [this]() {
        auto path = QFileDialog::getSaveFileName(this, QObject::tr("Save Script"), "", QObject::tr("CE Scripts (*.cea);;All Files (*)"));
        if (path.isEmpty()) return;
        QFile f(path);
    ...
    templateBtn->setText(QObject::tr("Templates ▾"));
    templateBtn->setPopupMode(QToolButton::InstantPopup);
    templateBtn->setToolTip(QObject::tr("Insert a code template (Ctrl+I in Cheat Engine)"));
    ...
    auto* autoInject = templateMenu->addAction(QObject::tr("Code injection at address (auto)..."));
    autoInject->setToolTip(QObject::tr("Read the process at an address and generate a complete "
    ...
    for (const auto& t : ce::builtinAaTemplates()) {
        ...
        connect(action, &QAction::triggered, this, [this, body, name]() {
            if (!editor_->toPlainText().trimmed().isEmpty()) {
                auto answer = QMessageBox::question(this, QObject::tr("Insert template?"),
                    QObject::tr("Replace the current script with the '%1' template?\n"
    ...
    {
        const ce::gui::EditorPalette pal = ce::gui::editorPalette();
        output_->setStyleSheet(QObject::tr("background: %1; color: %2;")
...
void ScriptEditor::onExecute() {
    ...
    if (!proc_ || !autoAsm_) {
        ...
        output_->append(QObject::tr("No process selected!"));
        return;
    }
    ...
    output_->append(QObject::tr("Executing..."));
    ...
    if (result.success) {
        ...
        output_->append(QObject::tr("Script executed successfully."));
        ...
        output_->setTextColor(ce::gui::editorPalette().error);
        output_->append(QObject::tr("FAILED: ") + QString::fromStdString(result.error));
    }
}
...
void ScriptEditor::onDisable() {
    ...
    output_->clear();
    output_->setTextColor(ce::gui::editorPalette().text);
    output_->append(QObject::tr("Disabling..."));
    ...
    if (result.success) {
        output_->setTextColor(ce::gui::editorPalette().success);
        output_->append(QObject::tr("Script disabled."));
        enabled_ = false;
...
void ScriptEditor::onGenerateCodeInjection() {
    if (!proc_) {
        QMessageBox::warning(this, QObject::tr("No process"), QObject::tr("Attach to a process first."));
        return;
    }
    bool ok = false;
    QString text = QInputDialog::getText(this, QObject::tr("Code injection"),
        QObject::tr("Address (hex) to inject at:"), QLineEdit::Normal, QObject::tr(""), &ok);
    ...
    if (!ok || address == 0) {
        QMessageBox::warning(this, QObject::tr("Bad address"), QObject::tr("Enter a valid hex address."));
        return;
    }
    ...
    if (script.empty()) {
        QMessageBox::warning(this, QObject::tr("Code injection"),
            QString::fromStdString(genErr.empty() ? "Could not generate a template here." : genErr));
        return;
    }

    if (!editor_->toPlainText().trimmed().isEmpty()) {
        auto answer = QMessageBox::question(this, QObject::tr("Replace script?"),
            QObject::tr("Replace the current script with the generated injection?"),
    ...
    output_->clear();
    output_->setTextColor(ce::gui::editorPalette().success);
    output_->append(QObject::tr("Generated code injection. Fill in the code at '// your code here'."));
}

void ScriptEditor::onAddToTable() {
    auto script = editor_->toPlainText();
    if (script.trimmed().isEmpty()) {
        QMessageBox::information(this, QObject::tr("Add to Cheat Table"), QObject::tr("The script is empty."));
        return;
    }
    if (!addToTable_) {
        QMessageBox::warning(this, QObject::tr("Add to Cheat Table"),
            QObject::tr("This editor isn't connected to a cheat table."));
        return;
    }
    bool ok = false;
    QString desc = QInputDialog::getText(this, QObject::tr("Add to Cheat Table"),
        QObject::tr("Description for the table entry:"), QLineEdit::Normal,
        defaultDescription_, &ok);
    if (!ok) return;
    addToTable_(desc, script);
    output_->setTextColor(ce::gui::editorPalette().success);
    output_->append(QObject::tr("Saved to the cheat table. Toggle its checkbox to enable/disable."));
}
...
void ScriptEditor::onCheck() {
    ...
    if (result.success) {
        output_->setTextColor(ce::gui::editorPalette().success);
        output_->append(QObject::tr("Syntax check passed."));
    }
}
