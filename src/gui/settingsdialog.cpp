...
SettingsDialog::SettingsDialog(QWidget* parent) : QDialog(parent) {
    ...
    addPage(buildScanTab(),       QObject::tr("Scan"));
    addPage(buildDisplayTab(),    QObject::tr("Display"));
    addPage(buildDebuggerTab(),   QObject::tr("Debugger"));
    addPage(buildMemoryViewTab(), QObject::tr("Memory View"));
    addPage(buildHotkeysTab(),    QObject::tr("Hotkeys"));
    addPage(buildNetworkTab(),    QObject::tr("Network"));
    ...
    // they need no onApply plumbing.
    {
        ...
        auto* demangle = new QCheckBox(QObject::tr("Demangle C++ symbol names"));
        ...
        sf->addRow("", demangle);
        auto* dwarf = new QCheckBox(QObject::tr("Load DWARF debug info when available"));
        ...
        sf->addRow("", dwarf);
        addPage(sym, QObject::tr("Symbols"));
        ...
        cff->addRow(QObject::tr("Default watch size (bytes):"), watch);
        addPage(cf, QObject::tr("CodeFinder"));

        auto* lua = new QWidget; auto* lf = new QFormLayout(lua);
        auto* luaWarn = new QLabel(
            QObject::tr("Unsafe Lua functions (shellExecute, write*Local) are disabled unless "
        ...
        noteTab(QObject::tr("Unrandomizer"), QObject::tr("ASLR unrandomizer (Windows-only in CE). On Linux, "
                                "disable ASLR per-launch with: setarch $(uname -m) -R <program>."));
        {
            auto* pl = new QWidget; auto* pf = new QFormLayout(pl);
            auto* pnote = new QLabel(QObject::tr("Native plugins (.so) load from the app directory. "
                                     "The Lua plugin API is available from the Lua console."));
            pnote->setWordWrap(true); pf->addRow(pnote);
            addPage(pl, QObject::tr("Plugins"));
        }
        {
            ...
            lgf->addRow(QObject::tr("Language:"), langCombo);
            auto* note = new QLabel(QObject::tr("Translations ship as translations/cheatengine_<locale>.qm; "
                                    "restart to apply."));
            note->setWordWrap(true); lgf->addRow(note);
            addPage(lg, QObject::tr("Languages"));
        }
        noteTab(QObject::tr("Extra"), QObject::tr("Miscellaneous options."));
        noteTab(QObject::tr("Tools"), QObject::tr("External tool integration (disassembler/hex editor paths)."));
        noteTab(QObject::tr("Signing"), QObject::tr("Cheat-table signing (Windows crypto in CE). Not applicable on Linux."));
    }
    ...
    btnRow->addStretch();
    auto* applyBtn = new QPushButton(QObject::tr("Apply"));
...
QWidget* SettingsDialog::buildScanTab() {
    ...
    form->addRow(QObject::tr("Default alignment:"), alignSpin_);

    writableCheck_ = new QCheckBox(QObject::tr("Scan writable memory only by default"));
    writableCheck_->setChecked(s.value(SCAN_WRITABLE_KEY, true).toBool());
    form->addRow("", writableCheck_);

    executableCheck_ = new QCheckBox(QObject::tr("Scan executable memory only by default"));
    executableCheck_->setChecked(s.value(SCAN_EXEC_KEY, false).toBool());
    form->addRow("", executableCheck_);

    fastScanCheck_ = new QCheckBox(QObject::tr("Fast scan (alignment-aware)"));
    ...
    form->addRow(QObject::tr("Worker threads:"), threadsSpin_);
    ...
    form->addRow(QObject::tr("Default value type:"), defaultValueTypeCombo_);
    ...
    form->addRow(QObject::tr("Default float tolerance:"), defaultFloatToleranceSpin_);
...
QWidget* SettingsDialog::buildDisplayTab() {
    ...
    QSettings s;
    hexUpperCheck_ = new QCheckBox(QObject::tr("Uppercase hex"));
    ...
    form->addRow(QObject::tr("Monospace font:"), fontFamilyCombo_);

    fontSizeSpin_ = new QSpinBox;
    fontSizeSpin_->setRange(7, 24);
    fontSizeSpin_->setValue(s.value(DISP_FONT_SIZE_KEY, 10).toInt());
    form->addRow(QObject::tr("Font size:"), fontSizeSpin_);

    addressWidthCombo_ = new QComboBox;
    addressWidthCombo_->addItems({QObject::tr("32-bit (8 hex chars)"), QObject::tr("64-bit (16 hex chars)")});
    addressWidthCombo_->setCurrentIndex(s.value(DISP_ADDR_WIDTH_KEY, 1).toInt());
    form->addRow(QObject::tr("Address width:"), addressWidthCombo_);

    darkThemeCheck_ = new QCheckBox(QObject::tr("Dark theme (Catppuccin)"));
...
QWidget* SettingsDialog::buildDebuggerTab() {
    ...
    form->addRow(QObject::tr("Default breakpoint type:"), bpTypeCombo_);

    bpSizeSpin_ = new QSpinBox;
    bpSizeSpin_->setRange(1, 8);
    bpSizeSpin_->setValue(s.value(DBG_BP_SIZE_KEY, 4).toInt());
    form->addRow(QObject::tr("Default data BP size (bytes):"), bpSizeSpin_);
    ...
    form->addRow(QObject::tr("ptrace attach settle delay:"), attachDelaySpin_);
    ...
    form->addRow(QObject::tr("Lua condition eval timeout:"), conditionTimeoutSpin_);

    breakOnSignalsCheck_ = new QCheckBox(QObject::tr("Break on SIGSEGV / SIGFPE / SIGILL"));
...
QWidget* SettingsDialog::buildMemoryViewTab() {
    ...
    form->addRow(QObject::tr("Hex bytes per row:"), bytesPerRowSpin_);
    ...
    form->addRow(QObject::tr("Auto-refresh interval:"), refreshIntervalSpin_);

    autoRefreshCheck_ = new QCheckBox(QObject::tr("Auto-refresh memory view"));
    ...
    form->addRow(QObject::tr("Default architecture:"), defaultArchCombo_);
...
QWidget* SettingsDialog::buildHotkeysTab() {
    ...
    layout->addWidget(new QLabel(
        QObject::tr("Per-table hotkeys are configured from the Address List right-click menu.\n"
        "These are global default values used when adding new entries.")));

    QSettings s;
    hotkeyTable_ = new QTableWidget;
    hotkeyTable_->setColumnCount(2);
    hotkeyTable_->setHorizontalHeaderLabels({QObject::tr("Setting"), QObject::tr("Value")});
    ...
    hotkeyTable_->setItem(0, 0, new QTableWidgetItem(QObject::tr("Default value step")));
...
QWidget* SettingsDialog::buildNetworkTab() {
    ...
    form->addRow(QObject::tr("ceserver host:"), ceserverHostEdit_);
    ...
    form->addRow(QObject::tr("ceserver port:"), ceserverPortSpin_);
    ...
    form->addRow(QObject::tr("zlib compression level:"), compressionLevelSpin_);
    ...
    form->addRow(QObject::tr("Default GDB stub port:"), gdbDefaultPortSpin_);
