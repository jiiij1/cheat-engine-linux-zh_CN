...
StackViewWindow::StackViewWindow(ProcessHandle* proc, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Stack Trace"));
    resize(850, 560);
    ...
    stackTable_->setHorizontalHeaderLabels({QObject::tr("Address"), QObject::tr("Value"), QObject::tr("Offset")});
    ...
    tabs_->addTab(stackTable_, QObject::tr("Raw Stack"));

    traceTable_ = new QTableWidget;
    traceTable_->setColumnCount(5);
    traceTable_->setHorizontalHeaderLabels({QObject::tr("Frame"), QObject::tr("Instruction"), QObject::tr("Return"), QObject::tr("Frame Pointer"), QObject::tr("Symbol")});
    ...
    tabs_->addTab(traceTable_, QObject::tr("Stack Trace"));
...
void StackViewWindow::refreshStack() {
    ...
    if (!attached) {
        statusLabel_->setText(QObject::tr("Could not attach to thread for stack context."));
        return;
    }
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        stackTable_->setItem(i, 1, new QTableWidgetItem(fullRead ? QString("%1").arg(value, 16, 16, QChar('0')) : QObject::tr("??")));
    ...
    statusLabel_->setText(QObject::tr("TID %1 RSP=0x%2 RBP=0x%3 Frames=%4")
        .arg(tid)
