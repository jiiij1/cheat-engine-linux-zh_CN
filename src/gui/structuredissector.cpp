...
StructureDissector::StructureDissector(ProcessHandle* proc, uintptr_t baseAddr, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Structure Dissector"));
    resize(750, 600);
    ...
    addrRow->addWidget(new QLabel(QObject::tr("Base Address:")));
    ...
    addrRow->addWidget(new QLabel(QObject::tr("Bytes:")));
    ...
    addrRow->addWidget(new QLabel(QObject::tr("Compare:")));
    compareEdit_ = new QLineEdit;
    compareEdit_->setFont(QFont("Monospace", 10));
    compareEdit_->setMinimumWidth(180);
    compareEdit_->setPlaceholderText(QObject::tr("0x…, 0x… (optional, comma/space separated)"));
    compareEdit_->setToolTip(
        QObject::tr("Enter one or more struct addresses to compare against the base. Rows whose "
    ...
    auto* saveBtn = new QPushButton(QObject::tr("Save Definition…"));
    saveBtn->setToolTip(QObject::tr("Save this structure definition (named fields + size)"));
    connect(saveBtn, &QPushButton::clicked, this, &StructureDissector::onSaveDefinition);
    actionRow->addWidget(saveBtn);
    auto* loadBtn = new QPushButton(QObject::tr("Load Definition…"));
    loadBtn->setToolTip(QObject::tr("Load a previously saved structure definition"));
    connect(loadBtn, &QPushButton::clicked, this, &StructureDissector::onLoadDefinition);
    actionRow->addWidget(loadBtn);
    auto* cppBtn = new QPushButton(QObject::tr("Copy as C++"));
    cppBtn->setToolTip(QObject::tr("Copy the current layout to the clipboard as a C++ struct"));
    connect(cppBtn, &QPushButton::clicked, this, &StructureDissector::onCopyAsCpp);
    actionRow->addWidget(cppBtn);
    auto* il2cppBtn = new QPushButton(QObject::tr("Type as IL2CPP…"));
    il2cppBtn->setToolTip(QObject::tr("Label these bytes with a Unity IL2CPP class's fields "
    ...
    auto* cstructBtn = new QPushButton(QObject::tr("Type as C struct…"));
    cstructBtn->setToolTip(QObject::tr("Label these bytes with a native struct from the target's DWARF debug info"));
    connect(cstructBtn, &QPushButton::clicked, this, &StructureDissector::onTypeAsCStruct);
    actionRow->addWidget(cstructBtn);
    auto* addAllBtn = new QPushButton(QObject::tr("Add All to List"));
    addAllBtn->setToolTip(QObject::tr("Add every named field (base+offset, name, guessed type) to the cheat table"));
    connect(addAllBtn, &QPushButton::clicked, this, [this]() {
        int n = addAllFieldsToList();
        statusBar()->showMessage(n > 0 ? QObject::tr("Added %1 field%2 to the cheat table")
                                             .arg(n).arg(n == 1 ? "" : "s")
                                       : QObject::tr("No named fields to add (double-click a row, or Type as…)"),
                                 4000);
    });
    ...
    table_->setHorizontalHeaderLabels({QObject::tr("Offset"), QObject::tr("Name"), QObject::tr("Hex"), QObject::tr("Int8"), QObject::tr("Int32"), "Float", QObject::tr("Pointer?")});
    ...
    connect(table_, &QTableWidget::cellDoubleClicked, this, [this](int row, int col) {
        ...
        QString name = QInputDialog::getText(this, QObject::tr("Name field"),
            QObject::tr("Name for offset +0x%1:").arg(off, 0, 16), QLineEdit::Normal, cur, &ok);
        if (!ok) return;
...
void StructureDissector::detachFromTarget() {
    ...
    setWindowTitle(QObject::tr("Structure Dissector (target exited)"));
}
...
void StructureDissector::onContextMenu(const QPoint& pos) {
    ...
    QMenu menu(this);
    menu.addAction(QObject::tr("Offset +0x%1  (0x%2)").arg(off, 2, 16, QChar('0')).arg(addr, 0, 16))
        ->setEnabled(false);
    menu.addSeparator();
    auto* addGuess = menu.addAction(QObject::tr("Add to address list as %1 (auto)").arg(typeName(guessed)));
    auto* add4  = menu.addAction(QObject::tr("Add as 4 Bytes"));
    auto* add8  = menu.addAction(QObject::tr("Add as 8 Bytes"));
    auto* addF  = menu.addAction(QObject::tr("Add as Float"));
    auto* addP  = menu.addAction(QObject::tr("Add as Pointer"));
    ...
    menu.addSeparator();
    auto* nameAct = menu.addAction(QObject::tr("Name field..."));
    auto* typeMenu = menu.addMenu(QObject::tr("Set field type"));
    ...
    auto* autoTypeAct = typeMenu->addAction(QObject::tr("Auto (guess)"));
...
void StructureDissector::addFieldToList(int offset, ce::ValueType type) {
    ...
    QString desc = (nit != fieldNames_.end() && !nit->second.isEmpty())
        ? nit->second : QObject::tr("Dissect +0x%1").arg(offset, 0, 16);
...
void StructureDissector::onSaveDefinition() {
    auto path = QFileDialog::getSaveFileName(this, QObject::tr("Save structure definition"),
        "structure.json", QObject::tr("Structure def (*.json);;All (*)"));
    if (path.isEmpty()) return;
    if (!saveDefinition(path))
        QMessageBox::warning(this, QObject::tr("Save failed"), QObject::tr("Could not write the definition file."));
}

void StructureDissector::onLoadDefinition() {
    auto path = QFileDialog::getOpenFileName(this, QObject::tr("Load structure definition"),
        "", QObject::tr("Structure def (*.json);;All (*)"));
    if (path.isEmpty()) return;
    if (!loadDefinition(path))
        QMessageBox::warning(this, QObject::tr("Load failed"),
            QObject::tr("Could not read the file or it is not a valid structure definition."));
}

void StructureDissector::onTypeAsIl2Cpp() {
    if (!proc_) { QMessageBox::information(this, "IL2CPP", QObject::tr("No target process.")); return; }
    bool ok = false;
    QString cls = QInputDialog::getText(this, QObject::tr("Type as IL2CPP class"),
        QObject::tr("Class name (e.g. UnityEngine.Vector3 or Player):"), QLineEdit::Normal, QObject::tr(""), &ok);
    ...
    if (!found) {
        QMessageBox::warning(this, "IL2CPP", QObject::tr("Class not found: ") + cls);
        return;
    }
...
void StructureDissector::onTypeAsCStruct() {
    if (!proc_) { QMessageBox::information(this, "DWARF", QObject::tr("No target process.")); return; }
    if (!ce::DwarfInfo::available()) {
        QMessageBox::information(this, "DWARF", QObject::tr("This build has no libdw (DWARF) support."));
        return;
    }
    bool ok = false;
    QString name = QInputDialog::getText(this, QObject::tr("Type as C struct"),
        QObject::tr("Struct name (from the target's debug info):"), QLineEdit::Normal, QObject::tr(""), &ok);
    ...
    QApplication::restoreOverrideCursor();
    if (!st) {
        QMessageBox::warning(this, "DWARF", QObject::tr("Struct not found (needs debug info): ") + name);
        return;
    }
...
void StructureDissector::populateTable() {
    ...
    if (table_->columnCount() != cols) {
        ...
        QStringList headers; headers << QObject::tr("Offset") << QObject::tr("Name") << QObject::tr("Base");
...
void StructureDissector::onCopyAsCpp() {
    if (fieldNames_.empty()) {
        QMessageBox::information(this, QObject::tr("Copy as C++"),
            QObject::tr("Name some fields first (double-click a row to name the offset)."));
        return;
    }
    ...
    QMessageBox::information(this, QObject::tr("Copy as C++"),
        QObject::tr("C++ struct copied to clipboard:\n\n") + cpp);
}
