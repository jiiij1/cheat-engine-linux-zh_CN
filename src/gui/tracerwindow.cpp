...
TracerWindow::TracerWindow(ProcessHandle* proc, DebuggerFactory factory, QWidget* parent)
    ...
    setWindowTitle(QObject::tr("Break and Trace"));
    resize(1100, 600);
    buildUi();
}
...
void TracerWindow::buildUi() {
    ...
    form->addWidget(new QLabel(QObject::tr("Start address (hex):")), row, 0);
    startAddressEdit_ = new QLineEdit;
    startAddressEdit_->setPlaceholderText(QObject::tr("0x0000000000000000, leave blank to start at RIP"));
    form->addWidget(startAddressEdit_, row, 1, 1, 3);
    ++row;

    form->addWidget(new QLabel(QObject::tr("Max steps:")), row, 0);
    ...
    stepOverCheck_ = new QCheckBox(QObject::tr("Step over calls"));
    form->addWidget(stepOverCheck_, row, 2);

    stayInModuleCheck_ = new QCheckBox(QObject::tr("Stay in module"));
    form->addWidget(stayInModuleCheck_, row, 3);
    ++row;

    form->addWidget(new QLabel(QObject::tr("Module start (hex):")), row, 0);
    moduleStartEdit_ = new QLineEdit;
    moduleStartEdit_->setPlaceholderText(QObject::tr("0x... (only if Stay in module)"));
    form->addWidget(moduleStartEdit_, row, 1);
    form->addWidget(new QLabel(QObject::tr("end:")), row, 2);
    moduleEndEdit_ = new QLineEdit;
    form->addWidget(moduleEndEdit_, row, 3);
    ++row;

    form->addWidget(new QLabel(QObject::tr("Stop address (hex, optional):")), row, 0);
    stopAddressEdit_ = new QLineEdit;
    stopAddressEdit_->setPlaceholderText(QObject::tr("Trace stops when this address is reached"));
    ...
    startBtn_ = new QPushButton(QObject::tr("Start Trace"));
    cancelBtn_ = new QPushButton(QObject::tr("Cancel"));
    cancelBtn_->setEnabled(false);
    saveBtn_ = new QPushButton(QObject::tr("Save…"));
    saveBtn_->setEnabled(false);
    progressBar_ = new QProgressBar;
    progressBar_->setRange(0, 100);
    statusLabel_ = new QLabel(QObject::tr("Idle"));
    ...
    table_->setHorizontalHeaderLabels({QObject::tr("#"), QObject::tr("Address"), QObject::tr("Instruction"), "RAX", "RBX", "RCX", "RDX", "RIP"});
    ...
    connect(table_, &QTableWidget::itemDoubleClicked, this, [this](QTableWidgetItem* item) {
        ...
        dlg->setWindowTitle(QObject::tr("Step %1 @ 0x%2").arg(item->row()).arg(e.address, 0, 16));
        dlg->resize(500, 320);
...
void TracerWindow::onStart() {
    if (!proc_) {
        QMessageBox::warning(this, QObject::tr("No process"), QObject::tr("Open a process before tracing."));
        return;
    }
    if (worker_.joinable()) return;
    ...
    if (!debugger_) {
        QMessageBox::warning(this, QObject::tr("No debugger"),
            QObject::tr("No debugger could be constructed for the attached process."));
        return;
    }
    entries_.clear();
    table_->setRowCount(0);
    progressBar_->setValue(0);
    statusLabel_->setText(QObject::tr("Tracing…"));
...
void TracerWindow::onCancel() {
    tracer_.cancel();
    statusLabel_->setText(QObject::tr("Cancelling…"));
}
...
void TracerWindow::onTraceFinished() {
    ...
    statusLabel_->setText(QObject::tr("%1 steps recorded%2")
        .arg(entries_.size())
...
void TracerWindow::onSave() {
    auto path = QFileDialog::getSaveFileName(this, QObject::tr("Save trace"), "trace.txt", QObject::tr("Text (*.txt)"));
    ...
    for (size_t i = 0; i < entries_.size(); ++i) {
        ...
        out << QString("%1\t").arg(i) << hexQ(e.address) << '\t'
            ...
            << "rax=" << hexQ(e.context.rax) << QObject::tr(" rbx=") << hexQ(e.context.rbx)
            << QObject::tr(" rcx=") << hexQ(e.context.rcx) << QObject::tr(" rdx=") << hexQ(e.context.rdx)
            << QObject::tr(" rip=") << hexQ(e.context.rip) << '\n';
    }
}
