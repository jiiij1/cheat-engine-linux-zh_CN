...
void WaylandGlobalShortcuts::onCreateSessionResponse(uint response,
    ...
    if (response != 0) {
        emit sessionFailed(QObject::tr("CreateSession response %1").arg(response));
        return;
    }
    sessionHandle_ = results.value(QStringLiteral("session_handle")).toString();
    if (sessionHandle_.isEmpty()) {
        emit sessionFailed(QObject::tr("no session_handle in response"));
        return;
    }
    emit sessionReady();
}
...
void WaylandGlobalShortcuts::onBindResponse(uint response, const QVariantMap&) {
    if (response == 0)
        ...
        emit sessionFailed(QObject::tr("BindShortcuts response %1").arg(response));
}
