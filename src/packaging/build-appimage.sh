...
# Injected into Mono/IL2CPP targets; the host searches <prefix>/lib too.
MONO_AGENT_LIB="$(ls "$BUILD_DIR"/libcecore_mono_agent.so 2>/dev/null | head -1)"
...
"$TOOLS/linuxdeploy" --appdir "$APPDIR" \
    ...
    ${MONO_AGENT_LIB:+--library "$MONO_AGENT_LIB"} \
    --desktop-file "$SCRIPT_DIR/cheatengine.desktop" \
    --icon-file "$SCRIPT_DIR/cheatengine.png" \
    --plugin qt \
    --output appimage

# libspeedhack.so is dlopen'd by the GUI from the executable's own directory
# (applicationDirPath()), not linked, so linuxdeploy cannot discover it: copy it
# in next to the binaries. Without it the Speedhack dialog reports "dlopen
# returned … but libspeedhack.so is not mapped in the target".
if [ -f "$BUILD_DIR/libspeedhack.so" ]; then
    cp -f "$BUILD_DIR/libspeedhack.so" "$APPDIR/usr/bin/"
fi
# Re-run the output step so the appended file is included in the AppImage.
